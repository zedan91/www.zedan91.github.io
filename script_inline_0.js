    ...
    const profileSettingsCancelButton = document.getElementById('profileSettingsCancelButton');
    ...
    const marketUserTools = document.getElementById('marketUserTools');
    ...
    const siteForgotPasswordForm = document.getElementById('siteForgotPasswordForm');
    ...
    const switchForgotToSignin = document.getElementById('switchForgotToSignin');
    ...
    const siteResetName = document.getElementById('siteResetName');
    const siteResetError = document.getElementById('siteResetError');
    const siteResetSubmitButton = document.getElementById('siteResetSubmitButton');
    ...
    const liveUsersPanel = document.getElementById('liveUsersPanel');
    const loginHistoryPanel = document.getElementById('loginHistoryPanel');
    const guestHistoryPanel = document.getElementById('guestHistoryPanel');
    ...
    async function ensureAuthSessionPersistence() {
      ...
      try {
        ...
        // Local persistence keeps login visible across new tabs; login can continue if the browser blocks it.
      }
    }
    ...
    function buildPdfUrl() {
      ...
      return `https://ebiz.jupem.gov.my/MuatTurunPembelian/MuatTurunPelanAkui?noPa=PA${number}.TIF&negeri=${encodeURIComponent(selectedNegeri)}`;
    }
    ...
    function updatePreview() {
      ...
      paLinkPreview.textContent = url || 'Link JUPEM akan dijana selepas nombor PA dan negeri dipilih.';
    }
    ...
    function getSavedUser() {
      return parseSavedUser(sessionStorage.getItem('azobssCurrentUser')) || parseSavedUser(localStorage.getItem('azobssCurrentUser'));
    }
    ...
    function hasSavedLogin() {
      return sessionStorage.getItem('azobssLoggedIn') === '1' || localStorage.getItem('azobssLoggedIn') === '1';
    }
    ...
    function showSignedInUser(user) {
      ...
      marketUserTools.style.display = 'flex';
    ...
    const luckyDrawParticipantsPanel = document.getElementById('luckyDrawParticipantsPanel');
    const luckyDrawParticipantsStatus = document.getElementById('luckyDrawParticipantsStatus');
    const luckyDrawPublicList = document.getElementById('luckyDrawPublicList');
    const luckyDrawAdminTableBody = document.getElementById('luckyDrawAdminTableBody');
    const luckyDrawParticipantSearch = document.getElementById('luckyDrawParticipantSearch');
    const refreshLuckyDrawParticipantsButton = document.getElementById('refreshLuckyDrawParticipantsButton');
    const exportLuckyDrawParticipantsButton = document.getElementById('exportLuckyDrawParticipantsButton');
    let allLuckyDrawParticipants = [];
    ...
    function formatLuckyDrawDate(value) {
      if (!value) return '-';
      if (typeof value === 'number') return new Date(value).toLocaleString('en-MY');
      if (value && typeof value.toDate === 'function') return value.toDate().toLocaleString('en-MY');
      return String(value);
    }

    
    function getLuckyDrawTimeValue(entry) {
      if (!entry) return 0;
      if (typeof entry.joinedAtMs === 'number') return entry.joinedAtMs;
      if (typeof entry.createdAtMs === 'number') return entry.createdAtMs;
      if (typeof entry.updatedAtMs === 'number') return entry.updatedAtMs;
      if (typeof entry.joinedAt === 'number') return entry.joinedAt;
      if (entry.joinedAt && typeof entry.joinedAt.toMillis === 'function') return entry.joinedAt.toMillis();
      if (entry.joinedAt && typeof entry.joinedAt.toDate === 'function') return entry.joinedAt.toDate().getTime();
      if (entry.createdAt && typeof entry.createdAt.toMillis === 'function') return entry.createdAt.toMillis();
      if (entry.createdAt && typeof entry.createdAt.toDate === 'function') return entry.createdAt.toDate().getTime();
      return 0;
    }

    function getLuckyDrawNewestRows(rows) {
      return (Array.isArray(rows) ? rows : []).slice().sort(function (a, b) {
        return getLuckyDrawTimeValue(b) - getLuckyDrawTimeValue(a);
      });
    }

    function getLuckyDrawSearchText(entry) {
      return [
        entry.monthKey,
        entry.usernameKey,
        entry.name,
    ...
    function getFilteredLuckyDrawParticipants() {
      const keyword = (luckyDrawParticipantSearch && luckyDrawParticipantSearch.value || '').trim().toLowerCase();
      const rows = getLuckyDrawNewestRows(allLuckyDrawParticipants).filter(function (entry) {
        return !entry.deleted;
      });

      if (!keyword || !isAdminUser(currentUser)) return rows;

      return rows.filter(function (entry) {
        return getLuckyDrawSearchText(entry).includes(keyword);
      });
    }

    function renderLuckyDrawParticipants() {
      allLuckyDrawParticipants = getLuckyDrawNewestRows(allLuckyDrawParticipants);
      const rows = getFilteredLuckyDrawParticipants();
      const admin = isAdminUser(currentUser);

      if (luckyDrawParticipantsStatus) {
        luckyDrawParticipantsStatus.textContent = admin
          ? `Jumlah peserta dipaparkan: ${rows.length}`
          : `Jumlah peserta Lucky Draw: ${rows.length}`;
      }

      if (luckyDrawPublicList) {
        luckyDrawPublicList.innerHTML = rows.length
          ? rows.map(function (entry, index) {
              return `
                <div class="lucky-draw-participant-card">
                  <strong>${joinNumber}. ${escapeLuckyDrawText(entry.name || entry.usernameKey || 'User')}</strong>
                  <span>${escapeLuckyDrawText(entry.monthKey || 'Lucky Draw')} â€¢ ${escapeLuckyDrawText(entry.usernameKey || '-')}</span>
                </div>
              `;
            }).join('')
      ...
      updateLuckyDrawParticipantCount();

      if (luckyDrawAdminTableBody) {
        luckyDrawAdminTableBody.innerHTML = rows.length
          ? rows.map(function (entry, index) {
              const rowId = escapeLuckyDrawText(entry.id || '');
              return `
                <tr data-entry-id="${rowId}">
                  <td>${joinNumber}</td>
                  ...
                  <td>${escapeLuckyDrawText(formatLuckyDrawDate(entry.joinedAtMs || entry.joinedAt || entry.createdAtMs))}</td>
                  <td>
                    <div class="lucky-draw-admin-actions">
                      <button class="lucky-draw-save-btn" type="button" data-action="save">Save</button>
                      <button class="lucky-draw-hide-btn" type="button" data-action="hide">Hide</button>
                    </div>
                  </td>
                </tr>
              `;
            }).join('')
          : '<tr><td colspan="9">Tiada peserta dijumpai.</td></tr>';
      }
    }

    async function loadLuckyDrawParticipants() {
      if (!luckyDrawParticipantsPanel) return;

      if (luckyDrawParticipantsStatus) luckyDrawParticipantsStatus.textContent = 'Loading peserta Lucky Draw...';

      try {
        if (!firebaseReady || !db) throw new Error('Firebase not ready');

        const snapshot = await getDocs(collection(db, 'luckyDrawEntries'));
        const rows = [];
        snapshot.forEach(function (item) {
          const data = item.data();
          if (!data.deleted) rows.push({ id: item.id, ...data });
        });
        ...
        allLuckyDrawParticipants = rows;
      } catch (error) {
        try {
          allLuckyDrawParticipants = JSON.parse(localStorage.getItem('azobssLuckyDrawEntries') || '[]')
            .map(function (entry, index) {
              return { id: entry.id || `${entry.monthKey || 'local'}_${entry.usernameKey || index}`, ...entry };
            })
            .filter(function (entry) {
              return !entry.deleted;
            });

          allLuckyDrawParticipants.sort(function (a, b) {
            return getLuckyDrawTimeValue(b) - getLuckyDrawTimeValue(a);
          });
        } catch (storageError) {
          allLuckyDrawParticipants = [];
        }
      }

      renderLuckyDrawParticipants();
    }

    async function saveLuckyDrawParticipant(row) {
      if (!row || !isAdminUser(currentUser)) return;

      const id = row.dataset.entryId;
      const entry = allLuckyDrawParticipants.find(function (item) {
        return item.id === id;
      });
      ...
      const payload = {
        ...
        contactEmail: row.querySelector('[data-field="contactEmail"]').value.trim(),
        inviteCode: row.querySelector('[data-field="inviteCode"]').value.trim(),
        editedBy: currentUser.usernameKey || currentUser.name || 'admin',
        editedAtMs: Date.now(),
        editedAt: serverTimestamp()
      };

      try {
        await setDoc(doc(db, 'luckyDrawEntries', id), payload, { merge: true });
        if (luckyDrawParticipantsStatus) luckyDrawParticipantsStatus.textContent = 'Peserta berjaya dikemaskini.';
        await loadLuckyDrawParticipants();
      } catch (error) {
        Object.assign(entry, payload);
        localStorage.setItem('azobssLuckyDrawEntries', JSON.stringify(allLuckyDrawParticipants));
        renderLuckyDrawParticipants();
        if (luckyDrawParticipantsStatus) luckyDrawParticipantsStatus.textContent = 'Firebase gagal save. Data local dikemaskini sementara.';
      }
    }

    async function hideLuckyDrawParticipant(row) {
      if (!row || !isAdminUser(currentUser)) return;
      ...
      const ok = confirm(`Hide peserta ${entry.name || entry.usernameKey}?`);
      if (!ok) return;

      try {
        await setDoc(doc(db, 'luckyDrawEntries', id), {
          deleted: true,
          deletedBy: currentUser.usernameKey || currentUser.name || 'admin',
          deletedAtMs: Date.now(),
          deletedAt: serverTimestamp()
        }, { merge: true });
        if (luckyDrawParticipantsStatus) luckyDrawParticipantsStatus.textContent = 'Peserta berjaya di-hide.';
        await loadLuckyDrawParticipants();
      } catch (error) {
        entry.deleted = true;
        localStorage.setItem('azobssLuckyDrawEntries', JSON.stringify(allLuckyDrawParticipants));
        renderLuckyDrawParticipants();
        if (luckyDrawParticipantsStatus) luckyDrawParticipantsStatus.textContent = 'Firebase gagal hide. Data local dikemaskini sementara.';
      }
    }

    function exportLuckyDrawParticipantsCsv() {
      if (!isAdminUser(currentUser) || !allLuckyDrawParticipants.length) return;

      const rows = getFilteredLuckyDrawParticipants();
      const headers = ['monthKey', 'usernameKey', 'name', 'phone', 'contactEmail', 'inviteCode', 'joinedAtMs'];
      const csv = [
        headers.join(','),
      ...
      a.href = url;
      a.download = `azobss-Lucky Draw-peserta-${new Date().toISOString().slice(0,10)}.csv`;
    ...
    if (luckyDrawParticipantSearch) luckyDrawParticipantSearch.addEventListener('input', renderLuckyDrawParticipants);
    if (refreshLuckyDrawParticipantsButton) refreshLuckyDrawParticipantsButton.addEventListener('click', loadLuckyDrawParticipants);
    if (exportLuckyDrawParticipantsButton) exportLuckyDrawParticipantsButton.addEventListener('click', exportLuckyDrawParticipantsCsv);

    if (luckyDrawAdminTableBody) {
      luckyDrawAdminTableBody.addEventListener('click', function (event) {
        const button = event.target.closest('button[data-action]');
        if (!button) return;

        const row = button.closest('tr[data-entry-id]');
        if (button.dataset.action === 'save') saveLuckyDrawParticipant(row);
        if (button.dataset.action === 'hide') hideLuckyDrawParticipant(row);
      });
    }



    const luckyDrawTotalParticipants = document.getElementById('luckyDrawTotalParticipants');
    const luckyDrawCountdown = document.getElementById('luckyDrawCountdown');
    const luckyDrawTargetText = document.getElementById('luckyDrawTargetText');
    const luckyDrawWinnerMonthLabel = document.getElementById('luckyDrawWinnerMonthLabel');
    const luckyDrawWinnerName = document.getElementById('luckyDrawWinnerName');
    const luckyDrawSpinStage = document.getElementById('luckyDrawSpinStage');
    const luckyDrawSpinnerName = document.getElementById('luckyDrawSpinnerName');
    let luckyDrawCountdownTimer = null;
    let luckyDrawSpinRunning = false;
    let currentLuckyDrawWinner = null;

    function getLuckyDrawMonthName(monthKey = getLuckyDrawMonthKey()) {
      const parts = String(monthKey).split('-');
      const year = Number(parts[0]);
      const monthIndex = Number(parts[1]) - 1;
      const date = new Date(year || new Date().getFullYear(), Number.isFinite(monthIndex) ? monthIndex : new Date().getMonth(), 1);
      return date.toLocaleDateString('ms-MY', { month: 'long', year: 'numeric' });
    }

    function getLuckyDrawTargetDate(monthKey = getLuckyDrawMonthKey()) {
      const parts = String(monthKey).split('-');
      const year = Number(parts[0]) || new Date().getFullYear();
      const month = Number(parts[1]) || (new Date().getMonth() + 1);
      return new Date(year, month, 0, 22, 0, 0, 0);
    }

    function getLuckyDrawWinnerDocId(monthKey = getLuckyDrawMonthKey()) {
      return monthKey;
    }

    function updateLuckyDrawParticipantCount() {
      const activeRows = allLuckyDrawParticipants.filter(function (entry) {
        return !entry.deleted;
      });
      if (luckyDrawTotalParticipants) luckyDrawTotalParticipants.textContent = String(activeRows.length);
    }

    function getWinnerDisplayName(entry) {
      return entry ? (entry.name || entry.usernameKey || 'User') : 'Belum dipilih';
    }

    function showLuckyDrawWinner(winner) {
      currentLuckyDrawWinner = winner || null;
      const monthName = getLuckyDrawMonthName(winner && winner.monthKey ? winner.monthKey : getLuckyDrawMonthKey());

      if (luckyDrawWinnerMonthLabel) luckyDrawWinnerMonthLabel.textContent = `Pemenang Bulan ${monthName}`;
      if (luckyDrawWinnerName) luckyDrawWinnerName.textContent = winner ? getWinnerDisplayName(winner) : 'Belum dipilih';

      if (luckyDrawSpinnerName) {
        luckyDrawSpinnerName.textContent = winner
          ? `ðŸŽ‰ ${getWinnerDisplayName(winner)} ðŸŽ‰`
          : '';
      }

      if (luckyDrawSpinStage) {
        luckyDrawSpinStage.classList.toggle('has-winner', !!winner);
        luckyDrawSpinStage.classList.remove('is-spinning');
      }
    }

    async function loadLuckyDrawWinner(monthKey = getLuckyDrawMonthKey()) {
      try {
        if (!firebaseReady || !db) throw new Error('Firebase not ready');
        const winnerSnap = await getDoc(doc(db, 'luckyDrawWinners', getLuckyDrawWinnerDocId(monthKey)));
        if (winnerSnap.exists()) {
          const winnerData = winnerSnap.data();
          if (!winnerData.deleted) {
            showLuckyDrawWinner({ id: winnerSnap.id, ...winnerData });
            return winnerData;
          }
        }
      } catch (error) {
        try {
          const winners = JSON.parse(localStorage.getItem('azobssLuckyDrawWinners') || '{}');
          if (winners[monthKey]) {
            showLuckyDrawWinner(winners[monthKey]);
            return winners[monthKey];
          }
        } catch (storageError) {}
      }

      showLuckyDrawWinner(null);
      return null;
    }

    function chooseLuckyDrawWinner(participants, monthKey) {
      const active = participants.filter(function (entry) {
        return !entry.deleted && entry.usernameKey;
      }).sort(function (a, b) {
        return String(a.usernameKey).localeCompare(String(b.usernameKey));
      });

      if (!active.length) return null;

      let seed = 0;
      const seedText = monthKey + '|' + active.map(function (entry) { return entry.usernameKey; }).join('|');
    ...
    async function saveLuckyDrawWinner(monthKey, winner) {
      ...
      const payload = {
        ...
        uid: winner.uid || '',
        name: winner.name || winner.usernameKey || 'User',
        phone: winner.phone || '',
      ...
      try {
        ...
        const winnerRef = doc(db, 'luckyDrawWinners', getLuckyDrawWinnerDocId(monthKey));
        const existing = await getDoc(winnerRef);
        if (existing.exists()) {
          const existingData = { id: existing.id, ...existing.data() };
          if (!existingData.deleted) {
            showLuckyDrawWinner(existingData);
            return existingData;
          }
        }

        await setDoc(winnerRef, { ...payload, deleted: false }, { merge: true });
        showLuckyDrawWinner(payload);
        return payload;
      } catch (error) {
        const winners = JSON.parse(localStorage.getItem('azobssLuckyDrawWinners') || '{}');
        winners[monthKey] = payload;
        localStorage.setItem('azobssLuckyDrawWinners', JSON.stringify(winners));
        showLuckyDrawWinner(payload);
        return payload;
      }
    }
    ...
    const luckyDrawRollingList = document.getElementById('luckyDrawRollingList');
    const luckyDrawConfetti = document.getElementById('luckyDrawConfetti');
    let luckyDrawAudioContext = null;

    function getLuckyDrawAudioContext() {
      const AudioCtx = window.AudioContext || window.webkitAudioContext;
      if (!AudioCtx) return null;
      if (!luckyDrawAudioContext) luckyDrawAudioContext = new AudioCtx();
      if (luckyDrawAudioContext.state === 'suspended') luckyDrawAudioContext.resume();
      return luckyDrawAudioContext;
    }

    function playLuckyDrawTickSound(step = 0) {
      const ctx = getLuckyDrawAudioContext();
      if (!ctx) return;
      ...
      osc.type = 'square';
      osc.frequency.setValueAtTime(520 + ((step % 7) * 34), ctx.currentTime);
      gain.gain.setValueAtTime(0.0001, ctx.currentTime);
      gain.gain.exponentialRampToValueAtTime(0.045, ctx.currentTime + 0.005);
    ...
    function playLuckyDrawWinnerSound() {
      const ctx = getLuckyDrawAudioContext();
      if (!ctx) return;
    ...
    function renderLuckyDrawRollingItems(names, selectedIndex = 0) {
      if (!luckyDrawRollingList) return;

      const safeNames = names.length ? names : ['Menunggu peserta...'];
      const items = [];

      for (let i = 0; i < 9; i += 1) {
        items.push(safeNames[(selectedIndex + i) % safeNames.length]);
      }

      luckyDrawRollingList.innerHTML = items.map(function(name){
        return `<span>${escapeLuckyDrawText ? escapeLuckyDrawText(name) : String(name)}</span>`;
      }).join('');

      luckyDrawRollingList.style.transition = 'none';
      luckyDrawRollingList.style.transform = 'translateY(0)';
    }

    function animateLuckyDrawRolling(names, step, itemHeight = 94) {
      if (!luckyDrawRollingList) return;

      renderLuckyDrawRollingItems(names, step);
      luckyDrawRollingList.style.transition = 'none';
      luckyDrawRollingList.style.transform = 'translateY(0)';

      requestAnimationFrame(function(){
        luckyDrawRollingList.style.transition = 'transform 80ms cubic-bezier(.2,.9,.2,1)';
        luckyDrawRollingList.style.transform = `translateY(-${itemHeight}px)`;
      });
    }

    function createLuckyDrawConfetti() {
      if (!luckyDrawConfetti) return;

      luckyDrawConfetti.innerHTML = '';
      const colors = ['#22c55e', '#fde047', '#3b82f6', '#f97316', '#ec4899', '#a855f7'];
      ...
      for (let i = 0; i < 42; i += 1) {
        ...
        luckyDrawConfetti.appendChild(piece);
      }
    ...
    function spinLuckyDrawNames(participants, finalWinner, monthKey) {
      if (!luckyDrawSpinnerName || !luckyDrawSpinStage || luckyDrawSpinRunning) return;

      const names = participants
        .filter(function (entry) { return !entry.deleted; })
        .map(getWinnerDisplayName);

      if (!names.length || !finalWinner) return;

      const itemHeight = window.matchMedia('(max-width: 760px)').matches ? 76 : 94;

      luckyDrawSpinRunning = true;
      luckyDrawSpinStage.classList.add('is-spinning');
      luckyDrawSpinStage.classList.remove('has-winner');
      luckyDrawSpinnerName.textContent = 'Spinning...';
      ...
      function nextTick() {
        ...
        animateLuckyDrawRolling(names, tick, itemHeight);
        luckyDrawSpinnerName.textContent = name;
        playLuckyDrawTickSound(tick);
        tick += 1;

        if (tick >= maxTicks) {
          luckyDrawSpinRunning = false;
          luckyDrawSpinStage.classList.remove('is-spinning');
          luckyDrawSpinStage.classList.add('has-winner');

          renderLuckyDrawRollingItems([getWinnerDisplayName(finalWinner)], 0);
          luckyDrawSpinnerName.textContent = `ðŸŽ‰ ${getWinnerDisplayName(finalWinner)} ðŸŽ‰`;
          playLuckyDrawWinnerSound();
          createLuckyDrawConfetti();
          saveLuckyDrawWinner(monthKey, finalWinner);
          return;
        }

        if (tick > 34) delay += 18;
        else if (tick > 22) delay += 8;
        else delay = Math.max(42, delay - 1);

        setTimeout(nextTick, delay);
      }

      renderLuckyDrawRollingItems(names, 0);
      nextTick();
    }

    async function runLuckyDrawIfDue() {
      const monthKey = getLuckyDrawMonthKey();
      const target = getLuckyDrawTargetDate(monthKey);
      const now = new Date();

      if (now < target) return;
      if (currentLuckyDrawWinner) return;

      await loadLuckyDrawParticipants();
      const existing = await loadLuckyDrawWinner(monthKey);
      if (existing) return;

      const winner = chooseLuckyDrawWinner(allLuckyDrawParticipants, monthKey);
      if (!winner) {
        if (luckyDrawSpinnerName) luckyDrawSpinnerName.textContent = 'Tiada peserta untuk spin bulan ini.';
        return;
      }

      spinLuckyDrawNames(allLuckyDrawParticipants, winner, monthKey);
    }

    function updateLuckyDrawCountdown() {
      if (!luckyDrawCountdown) return;
      ...
      if (luckyDrawTargetText) {
        luckyDrawTargetText.innerHTML = `Lucky Draw akan diadakan pada ${target.toLocaleDateString('ms-MY')}<br>jam 10:00 PM (UTC+08:00)`;
      }

      if (diff <= 0) {
        luckyDrawCountdown.textContent = '00 hari 00:00:00';
        runLuckyDrawIfDue();
        return;
      }
      ...
      luckyDrawCountdown.textContent =
    ...
    function startLuckyDrawCountdown() {
      if (luckyDrawCountdownTimer) clearInterval(luckyDrawCountdownTimer);
      updateLuckyDrawCountdown();
      luckyDrawCountdownTimer = setInterval(updateLuckyDrawCountdown, 1000);
      loadLuckyDrawWinner();
    }



    const testLuckyDrawSpinButton = document.getElementById('testLuckyDrawSpinButton');
    const resetLuckyDrawWinnerButton = document.getElementById('resetLuckyDrawWinnerButton');

    async function testLuckyDrawSpinNow() {
      if (!isAdminUser(currentUser)) return;

      await loadLuckyDrawParticipants();

      const active = allLuckyDrawParticipants.filter(function (entry) {
        return !entry.deleted && entry.usernameKey;
      });

      if (!active.length) {
        if (luckyDrawParticipantsStatus) luckyDrawParticipantsStatus.textContent = 'Tiada peserta untuk test spin.';
        if (luckyDrawSpinnerName) luckyDrawSpinnerName.textContent = 'Tiada peserta untuk spin.';
        return;
      }

      const randomWinner = active[Math.floor(Math.random() * active.length)];
      const names = active.map(getWinnerDisplayName);
      const itemHeight = window.matchMedia('(max-width: 760px)').matches ? 76 : 94;

      if (luckyDrawParticipantsStatus) {
        luckyDrawParticipantsStatus.textContent = 'Test spin sedang berjalan...';
      }
      ...
      function testTick() {
        ...
        if (tick >= maxTicks) {
          ...
          luckyDrawSpinStage.classList.remove('is-spinning');
          luckyDrawSpinStage.classList.add('has-winner');
          renderLuckyDrawRollingItems([`TEST ðŸŽ‰ ${getWinnerDisplayName(randomWinner)} ðŸŽ‰`], 0);
          luckyDrawSpinnerName.textContent = `TEST ðŸŽ‰ ${getWinnerDisplayName(randomWinner)} ðŸŽ‰`;
          playLuckyDrawWinnerSound();
          createLuckyDrawConfetti();

          if (luckyDrawParticipantsStatus) {
            luckyDrawParticipantsStatus.textContent = 'Test spin selesai. Pemenang test tidak disimpan.';
          }
          return;
        }

        if (tick > 30) delay += 18;
        else if (tick > 18) delay += 8;
    ...
    async function resetLuckyDrawWinner() {
      if (!isAdminUser(currentUser)) return;

      const monthKey = getLuckyDrawMonthKey();
      const ok = confirm(`Reset pemenang Lucky Draw bulan ${getLuckyDrawMonthName(monthKey)}?`);
      if (!ok) return;

      try {
        if (!firebaseReady || !db) throw new Error('Firebase not ready');

        // Soft reset: simpan rekod lama sebagai reset supaya admin masih ada audit.
        await setDoc(doc(db, 'luckyDrawWinners', getLuckyDrawWinnerDocId(monthKey)), {
          deleted: true,
          resetBy: currentUser.usernameKey || currentUser.name || 'admin',
          resetAtMs: Date.now(),
        ...
        try {
          const winners = JSON.parse(localStorage.getItem('azobssLuckyDrawWinners') || '{}');
          delete winners[monthKey];
          localStorage.setItem('azobssLuckyDrawWinners', JSON.stringify(winners));
        } catch(storageError){}
      }

      currentLuckyDrawWinner = null;

      if (luckyDrawWinnerName) luckyDrawWinnerName.textContent = 'Belum dipilih';
      if (luckyDrawWinnerMonthLabel) luckyDrawWinnerMonthLabel.textContent = `Pemenang Bulan ${getLuckyDrawMonthName(monthKey)}`;
      if (luckyDrawSpinnerName) luckyDrawSpinnerName.textContent = 'Winner sudah di-reset. Menunggu spin sebenar.';
      if (luckyDrawSpinStage) {
        luckyDrawSpinStage.classList.remove('has-winner', 'is-spinning');
      }

      if (luckyDrawParticipantsStatus) {
        luckyDrawParticipantsStatus.textContent = 'Pemenang bulan ini sudah di-reset.';
      }
    }

    if (testLuckyDrawSpinButton) testLuckyDrawSpinButton.addEventListener('click', testLuckyDrawSpinNow);
    if (resetLuckyDrawWinnerButton) resetLuckyDrawWinnerButton.addEventListener('click', resetLuckyDrawWinner);



    const resetLuckyDrawParticipantsButton = document.getElementById('resetLuckyDrawParticipantsButton');

    const luckyDrawPrizeImage = document.getElementById('luckyDrawPrizeImage');
    const luckyDrawPrizeTitle = document.getElementById('luckyDrawPrizeTitle');
    const luckyDrawPrizeDescription = document.getElementById('luckyDrawPrizeDescription');
    const luckyDrawPrizeImageFile = document.getElementById('luckyDrawPrizeImageFile');
    const luckyDrawPrizeImageDataInput = document.getElementById('luckyDrawPrizeImageDataInput');
    const luckyDrawPrizeTitleInput = document.getElementById('luckyDrawPrizeTitleInput');
    const luckyDrawPrizeDescriptionInput = document.getElementById('luckyDrawPrizeDescriptionInput');
    const saveLuckyDrawPrizeButton = document.getElementById('saveLuckyDrawPrizeButton');
    const refreshLuckyDrawPrizeButton = document.getElementById('refreshLuckyDrawPrizeButton');
    const luckyDrawPrizeSaveStatus = document.getElementById('luckyDrawPrizeSaveStatus');

    function setLuckyDrawPrizeStatus(message) {
      if (luckyDrawPrizeSaveStatus) luckyDrawPrizeSaveStatus.textContent = message || '';
      else if (luckyDrawParticipantsStatus) luckyDrawParticipantsStatus.textContent = message || '';
    }

    function getLuckyDrawPrizePlaceholder() {
      return 'https://placehold.co/600x400/0f172a/22c55e?text=Lucky+Draw+Prize';
    }

    function applyLuckyDrawPrize(data = {}) {
      const image = data.imageData || data.imageUrl || getLuckyDrawPrizePlaceholder();
      const title = data.title || 'Hadiah Lucky Draw bulan ini masih belum diumumkan.';
      const desc = data.description || 'Admin boleh browse gambar dan isi description hadiah Lucky Draw.';

      if (luckyDrawPrizeImage) luckyDrawPrizeImage.src = image;
      if (luckyDrawPrizeTitle) luckyDrawPrizeTitle.textContent = title;
      if (luckyDrawPrizeDescription) luckyDrawPrizeDescription.innerText = desc;

      if (luckyDrawPrizeImageDataInput) luckyDrawPrizeImageDataInput.value = data.imageData || data.imageUrl || '';
      if (luckyDrawPrizeTitleInput) luckyDrawPrizeTitleInput.value = data.title || '';
      if (luckyDrawPrizeDescriptionInput) luckyDrawPrizeDescriptionInput.value = data.description || '';
    }

    async function loadLuckyDrawPrize() {
      const monthKey = getLuckyDrawMonthKey();

      try {
        if (!firebaseReady || !db) throw new Error('Firebase not ready');
        const snap = await getDoc(doc(db, 'luckyDrawPrizes', monthKey));

        if (snap.exists()) {
          const data = snap.data();
          applyLuckyDrawPrize(data);
          setLuckyDrawPrizeStatus('');
          return data;
        }
      } catch (error) {
        try {
          const all = JSON.parse(localStorage.getItem('azobssLuckyDrawPrizes') || '{}');
          if (all[monthKey]) {
            applyLuckyDrawPrize(all[monthKey]);
            return all[monthKey];
          }
        } catch(storageError){}
      }
    ...
    if (luckyDrawPrizeImageFile) {
      luckyDrawPrizeImageFile.addEventListener('change', function () {
        const file = luckyDrawPrizeImageFile.files && luckyDrawPrizeImageFile.files[0];
        if (!file) return;

        if (!file.type.startsWith('image/')) {
          setLuckyDrawPrizeStatus('Sila pilih file gambar sahaja.');
          luckyDrawPrizeImageFile.value = '';
          return;
        }

        if (file.size > 650000) {
          setLuckyDrawPrizeStatus('Gambar terlalu besar. Sila guna gambar bawah 650KB supaya boleh save stabil ke Firebase.');
          luckyDrawPrizeImageFile.value = '';
          return;
        }
        ...
        reader.onload = function () {
          ...
          if (luckyDrawPrizeImageDataInput) luckyDrawPrizeImageDataInput.value = dataUrl;
          if (luckyDrawPrizeImage) luckyDrawPrizeImage.src = dataUrl;
          setLuckyDrawPrizeStatus('Gambar sudah dipilih. Tekan Save Prize untuk simpan ke backend.');
        };
        reader.readAsDataURL(file);
      });
    }

    async function saveLuckyDrawPrize() {
      if (!isAdminUser(currentUser)) {
        setLuckyDrawPrizeStatus('Admin sahaja boleh save hadiah.');
        return;
      }
      ...
      const payload = {
        monthKey,
        imageData: luckyDrawPrizeImageDataInput ? luckyDrawPrizeImageDataInput.value.trim() : '',
        title: luckyDrawPrizeTitleInput ? luckyDrawPrizeTitleInput.value.trim() : '',
        description: luckyDrawPrizeDescriptionInput ? luckyDrawPrizeDescriptionInput.value.trim() : '',
        updatedAtMs: Date.now(),
        updatedAt: serverTimestamp(),
        updatedBy: currentUser.usernameKey || currentUser.name || 'admin'
      };

      try {
        if (!firebaseReady || !db) throw new Error('Firebase not ready');
        await setDoc(doc(db, 'luckyDrawPrizes', monthKey), payload, { merge:true });
        setLuckyDrawPrizeStatus('Hadiah berjaya disimpan ke Firebase.');
      } catch(error) {
        const all = JSON.parse(localStorage.getItem('azobssLuckyDrawPrizes') || '{}');
        all[monthKey] = { ...payload, updatedAt: Date.now() };
        localStorage.setItem('azobssLuckyDrawPrizes', JSON.stringify(all));
        setLuckyDrawPrizeStatus('Firebase gagal/disekat. Hadiah disimpan local sementara.');
      }

      applyLuckyDrawPrize(payload);
    }

    if (saveLuckyDrawPrizeButton) saveLuckyDrawPrizeButton.addEventListener('click', saveLuckyDrawPrize);
    if (refreshLuckyDrawPrizeButton) refreshLuckyDrawPrizeButton.addEventListener('click', loadLuckyDrawPrize);

    async function resetLuckyDrawParticipantsThisMonth() {
      if (!isAdminUser(currentUser)) return;

      const monthKey = getLuckyDrawMonthKey();
      const ok = confirm(`Reset semua peserta yang join bulan ${getLuckyDrawMonthName(monthKey)}?`);
      if (!ok) return;
      ...
      try {
        ...
        snapshot.forEach(function (item) {
          const data = item.data();
          if (data.monthKey === monthKey && !data.deleted) {
            updates.push(setDoc(doc(db, 'luckyDrawEntries', item.id), {
              deleted: true,
              resetBy: currentUser.usernameKey || currentUser.name || 'admin',
              resetAtMs: Date.now(),
              resetAt: serverTimestamp()
        ...
        if (luckyDrawParticipantsStatus) luckyDrawParticipantsStatus.textContent = `Reset peserta selesai. User kini boleh join semula. Jumlah reset: ${updates.length}`;
      } catch(error) {
        try {
          const localEntries = JSON.parse(localStorage.getItem('azobssLuckyDrawEntries') || '[]');
          localEntries.forEach(function (entry) {
            if (entry.monthKey === monthKey) entry.deleted = true;
          });
          localStorage.setItem('azobssLuckyDrawEntries', JSON.stringify(localEntries));
          if (luckyDrawParticipantsStatus) luckyDrawParticipantsStatus.textContent = 'Peserta bulan ini di-reset daripada local data.';
        } catch(storageError) {
          if (luckyDrawParticipantsStatus) luckyDrawParticipantsStatus.textContent = 'Gagal reset peserta.';
        }
      }

      allLuckyDrawParticipants = allLuckyDrawParticipants.filter(function (entry) {
        return entry.monthKey !== monthKey;
      });

      await clearLuckyDrawJoinForReset(monthKey);

      try {
        Object.keys(localStorage).forEach(function(key){
          if(key.includes('azobssLuckyDrawJoined_' + monthKey)){
      ...
      if (typeof updateLuckyDrawPanel === 'function') {
        updateLuckyDrawPanel();
      }

      if (joinLuckyDrawButton) {
        joinLuckyDrawButton.disabled = false;
      }

      if (luckyDrawSpinnerName) {
        luckyDrawSpinnerName.textContent = 'Peserta bulan ini sudah di-reset. User boleh join semula.';
      }
    }

    if (resetLuckyDrawParticipantsButton) {
      resetLuckyDrawParticipantsButton.addEventListener('click', resetLuckyDrawParticipantsThisMonth);
    }

    function startLuckyDrawCountdownFixed() {
      if (luckyDrawCountdownTimer) clearInterval(luckyDrawCountdownTimer);
      updateLuckyDrawCountdown();
      luckyDrawCountdownTimer = setInterval(updateLuckyDrawCountdown, 1000);
      loadLuckyDrawWinner();
      loadLuckyDrawPrizeFromGithubFolder();
    }



    async function clearLuckyDrawJoinForReset(monthKey = getLuckyDrawMonthKey()) {
      try {
        const storageKey = `azobssLuckyDrawJoined_${monthKey}`;
        localStorage.removeItem(storageKey);
        sessionStorage.removeItem(storageKey);

        if (currentUser && currentUser.usernameKey) {
          localStorage.removeItem(`azobssLuckyDrawJoined_${monthKey}_${currentUser.usernameKey}`);
          sessionStorage.removeItem(`azobssLuckyDrawJoined_${monthKey}_${currentUser.usernameKey}`);
        }
      } catch(error){}
    }
    ...
    const downloadLuckyDrawPrizeJsonButton = document.getElementById('downloadLuckyDrawPrizeJsonButton');
    let luckyDrawSelectedPrizeFileName = '';

    function getLuckyDrawPrizeJsonPath() {
      return 'lucky-draw/Lucky Draw-prize.json?v=' + Date.now();
    }

    function getLuckyDrawPrizePlaceholderPath() {
      return 'lucky-draw/prize-placeholder.svg';
    }

    async function loadLuckyDrawPrizeFromGithubFolder() {
      try {
        const response = await fetch(getLuckyDrawPrizeJsonPath(), { cache: 'no-store' });
        if (!response.ok) throw new Error('JSON not found');
        const data = await response.json();

        const imagePath = data.image || data.imageUrl || getLuckyDrawPrizePlaceholderPath();
        if (luckyDrawPrizeImage) luckyDrawPrizeImage.src = imagePath;
        if (luckyDrawPrizeTitle) luckyDrawPrizeTitle.textContent = data.title || 'Hadiah Lucky Draw bulan ini masih belum diumumkan.';
        if (luckyDrawPrizeDescription) luckyDrawPrizeDescription.innerText = data.description || '';

        if (luckyDrawPrizeTitleInput) luckyDrawPrizeTitleInput.value = data.title || '';
        if (luckyDrawPrizeDescriptionInput) luckyDrawPrizeDescriptionInput.value = data.description || '';
        if (luckyDrawPrizeImageDataInput) luckyDrawPrizeImageDataInput.value = data.image || '';
        ...
        const fallback = {
          ...
          description: 'File lucky-draw/Lucky Draw-prize.json belum dijumpai atau belum upload ke GitHub.',
          image: getLuckyDrawPrizePlaceholderPath()
        };

        if (luckyDrawPrizeImage) luckyDrawPrizeImage.src = fallback.image;
        if (luckyDrawPrizeTitle) luckyDrawPrizeTitle.textContent = fallback.title;
        if (luckyDrawPrizeDescription) luckyDrawPrizeDescription.innerText = fallback.description;
        setLuckyDrawPrizeStatus('Hadiah dibaca dari folder /lucky-draw/. Upload Lucky Draw-prize.json untuk publish.');
        return fallback;
      }
    }

    function buildLuckyDrawPrizeJsonPayload() {
      const cleanFileName = luckyDrawSelectedPrizeFileName || 'prize-placeholder.svg';
      const imagePath = cleanFileName.startsWith('lucky-draw/')
        ? cleanFileName
        : 'lucky-draw/' + cleanFileName;

      return {
        monthKey: getLuckyDrawMonthKey(),
        title: luckyDrawPrizeTitleInput ? luckyDrawPrizeTitleInput.value.trim() : '',
        description: luckyDrawPrizeDescriptionInput ? luckyDrawPrizeDescriptionInput.value.trim() : '',
        image: imagePath,
        updatedAt: new Date().toISOString(),
        updatedBy: currentUser && (currentUser.usernameKey || currentUser.name) ? (currentUser.usernameKey || currentUser.name) : 'admin'
      };
    }
    ...
    function generateLuckyDrawPrizeJson() {
      if (!isAdminUser(currentUser)) {
        setLuckyDrawPrizeStatus('Admin sahaja boleh generate JSON hadiah.');
        return;
      }

      const payload = buildLuckyDrawPrizeJsonPayload();

      if (luckyDrawPrizeImage) {
        if (luckyDrawPrizeImageDataInput && luckyDrawPrizeImageDataInput.value && luckyDrawPrizeImageDataInput.value.startsWith('data:image/')) {
          luckyDrawPrizeImage.src = luckyDrawPrizeImageDataInput.value;
        } else {
          luckyDrawPrizeImage.src = payload.image;
        }
      }

      if (luckyDrawPrizeTitle) luckyDrawPrizeTitle.textContent = payload.title || 'Hadiah Lucky Draw';
      if (luckyDrawPrizeDescription) luckyDrawPrizeDescription.innerText = payload.description || '';

      const pretty = JSON.stringify(payload, null, 2);
      localStorage.setItem('azobssLuckyDrawPrizeGeneratedJson', pretty);

      setLuckyDrawPrizeStatus('Tekan Save Prize untuk simpan hadiah ke backend Render.');
    }

    function downloadLuckyDrawPrizeJson() {
      if (!isAdminUser(currentUser)) {
        setLuckyDrawPrizeStatus('Admin sahaja boleh download JSON hadiah.');
        return;
      }

      const payload = buildLuckyDrawPrizeJsonPayload();
      downloadTextFile('Lucky Draw-prize.json', JSON.stringify(payload, null, 2), 'application/json;charset=utf-8');

      setLuckyDrawPrizeStatus('JSON download dinyahaktifkan. Gunakan Save Prize untuk simpan ke backend Render.');
    }
    ...
    if (luckyDrawPrizeImageFile) {
      luckyDrawPrizeImageFile.addEventListener('change', function () {
        ...
        luckyDrawSelectedPrizeFileName = file.name.replace(/\s+/g, '-').replace(/[^a-zA-Z0-9._-]/g, '');
        ...
        reader.onload = function () {
          ...
          if (luckyDrawPrizeImageDataInput) luckyDrawPrizeImageDataInput.value = dataUrl;
          if (luckyDrawPrizeImage) luckyDrawPrizeImage.src = dataUrl;
          setLuckyDrawPrizeStatus(`Gambar dipilih: ${luckyDrawSelectedPrizeFileName}. Tekan Save Prize untuk simpan ke backend.`);
        };
        reader.readAsDataURL(file);
      }, true);
    }

    if (saveLuckyDrawPrizeButton) {
      saveLuckyDrawPrizeButton.addEventListener('click', function (event) {
        event.preventDefault();
        event.stopImmediatePropagation();
        generateLuckyDrawPrizeJson();
      }, true);
    }

    if (downloadLuckyDrawPrizeJsonButton) {
      downloadLuckyDrawPrizeJsonButton.addEventListener('click', function (event) {
        event.preventDefault();
        event.stopImmediatePropagation();
        downloadLuckyDrawPrizeJson();
      }, true);
    }

    if (refreshLuckyDrawPrizeButton) {
      refreshLuckyDrawPrizeButton.addEventListener('click', function (event) {
        event.preventDefault();
        event.stopImmediatePropagation();
        loadLuckyDrawPrizeFromGithubFolder();
      }, true);
          ...
          <div class="purchase-summary-item user-edit-item" data-user-key="${escapeHtml(usernameKey)}">
            ...
            <div class="user-edit-details">
              ...
              <div class="user-edit-actions">
                ...
                ${isAdminRecord ? '' : `<button class="small-action-btn" type="button" data-reset-password="${escapeHtml(usernameKey)}">Reset Password</button>`}
              </div>
            </div>
          </div>
        `;
    ...
    function generateTemporaryPassword(length = 10) {
      ...
      if (window.crypto && window.crypto.getRandomValues) {
        ...
        for (let i = 0; i < length; i += 1) password += chars[values[i] % chars.length];
      } else {
        for (let i = 0; i < length; i += 1) password += chars[Math.floor(Math.random() * chars.length)];
      }
      return password;
    }

    async function adminResetUserPassword(usernameKey) {
      if (!isAdminUser(currentUser) || !usernameKey || usernameKey === ADMIN_USERNAME) {
        return;
      }

      const newPassword = generateTemporaryPassword();
      if (!confirm(`Reset password for ${usernameKey}? A temporary password will be generated.`)) {
        return;
      }

      try {
        registeredUsersError.textContent = 'Resetting password...';
        registeredUsersError.style.color = '#bfdbfe';
        ...
        const res = await fetch('/api/admin-reset-user-password', {
          ...
          body: JSON.stringify({
            usernameKey,
            newPassword,
            requestedBy: currentUser.usernameKey
          })
        });
        ...
        registeredUsersError.style.color = '#bbf7d0';
        registeredUsersError.textContent = `Password reset for ${usernameKey}. Temporary password: ${newPassword}`;
      } catch (error) {
        registeredUsersError.style.color = '#fecaca';
        registeredUsersError.textContent = `${error.message || 'Reset password failed.'} Backend requires Firebase Admin service account.`;
      }
    }
    ...
    function syncAdminLivePanelsVisibility() {
      const shouldShow = isAdminUser(currentUser);
      document.body.classList.toggle('is-admin', shouldShow);
      if (registeredUsersPanel) {
        registeredUsersPanel.hidden = !shouldShow;
        registeredUsersPanel.style.display = shouldShow ? 'block' : 'none';
        registeredUsersPanel.style.visibility = shouldShow ? 'visible' : '';
        registeredUsersPanel.style.opacity = shouldShow ? '1' : '';
      }
      [liveUsersPanel, loginHistoryPanel, guestHistoryPanel].forEach(function (panel) {
        if (panel) {
          panel.hidden = !shouldShow;
          panel.style.display = shouldShow ? 'block' : 'none';
          panel.style.visibility = shouldShow ? 'visible' : '';
          panel.style.opacity = shouldShow ? '1' : '';
        }
      });
    }
    ...
    function clearSiteAuthInputs() {
      ...
      siteForgotPasswordForm.reset();
      siteLoginError.textContent = '';
      siteSignupError.textContent = '';
      siteResetError.textContent = '';
      siteResetError.style.color = '#fecaca';
    ...
    function openSiteAuth(mode = 'signin') {
      ...
      siteAuthTitle.textContent = isForgot ? 'Reset password' : (isSignup ? 'Sign up' : 'Sign in');
      siteSignInForm.hidden = isSignup || isForgot;
      siteSignUpForm.hidden = !isSignup;
      siteForgotPasswordForm.hidden = !isForgot;
      siteLoginError.textContent = '';
      siteSignupError.textContent = '';
      siteResetError.textContent = '';
      siteResetError.style.color = '#fecaca';
      ...
      setTimeout(function () {
        (mode === 'upgrade' && isLoggedIn ? memberUpgradeCode : (isForgot ? siteResetName : (isSignup ? siteSignupName : siteLoginName))).focus();
      }, 50);
    }
    ...
    async function requestSitePasswordReset() {
      const usernameKey = normalizeUsername(siteResetName.value || '');

      if (!usernameKey) {
        siteResetError.style.color = '#fecaca';
        siteResetError.textContent = 'Please enter your username.';
        return;
      }

      if (!firebaseReady || !auth) {
        siteResetError.style.color = '#fecaca';
        siteResetError.textContent = 'Firebase belum disambungkan.';
        return;
      }

      try {
        setButtonLoading(siteResetSubmitButton, true, 'Sending...', 'Send reset link');
        await sendPasswordResetEmail(auth, buildUserEmail(usernameKey));
        siteResetError.style.color = '#bbf7d0';
        siteResetError.textContent = 'Reset link sent. Check your email inbox/spam. If you do not receive it, ask admin to reset your password.';
      } catch (error) {
        siteResetError.style.color = '#fecaca';
        if (error.code === 'auth/user-not-found') {
          siteResetError.textContent = 'Username not found.';
        } else {
          siteResetError.textContent = 'Cannot send reset email. Ask admin to reset password for this username.';
        }
      } finally {
        setButtonLoading(siteResetSubmitButton, false, 'Sending...', 'Send reset link');
      }
    }
    ...
    function openProfileSettings() {
      ...
      profileEditName.value = currentUser.name || currentUser.usernameKey || '';
      profileEditPhone.value = currentUser.phone || '';
      profileEditEmail.value = currentUser.contactEmail || '';
      profileEditPassword.value = '';
      profileSettingsError.textContent = '';
      profileSettingsError.style.color = '#fecaca';
    ...
    async function saveProfileSettings() {
      if (!currentUser || !currentUser.usernameKey) {
        profileSettingsError.textContent = 'Please sign in first.';
        return;
      }

      const name = profileEditName.value.trim();
      const phone = profileEditPhone.value.trim();
      const contactEmail = profileEditEmail.value.trim();
      const newPassword = profileEditPassword.value.trim();

      if (!name) {
        profileSettingsError.textContent = 'Name cannot be empty.';
        return;
      }

      if (contactEmail && !isValidEmail(contactEmail)) {
        profileSettingsError.textContent = 'Please enter a valid email address.';
        return;
      }

      if (newPassword && newPassword.length < 6) {
        profileSettingsError.textContent = 'New password must be at least 6 characters.';
        return;
      }

      try {
        setButtonLoading(profileSettingsSaveButton, true, 'Saving...', 'Save Changes');
        profileSettingsError.textContent = '';
        ...
        saveCurrentUser(updatedUser);
        setLoggedIn(updatedUser);
        profileSettingsError.style.color = '#bbf7d0';
        profileSettingsError.textContent = newPassword ? 'Profile and password updated successfully.' : 'Profile updated successfully.';

        setTimeout(closeProfileSettings, 800);
      } catch (error) {
        profileSettingsError.style.color = '#fecaca';
        if (error.code === 'auth/requires-recent-login') {
          profileSettingsError.textContent = 'Please log out and log in again before changing password.';
        } else {
          profileSettingsError.textContent = `Failed to update profile: ${error.code || error.message}`;
        }
      } finally {
        setButtonLoading(profileSettingsSaveButton, false, 'Saving...', 'Save Changes');
      }
    }
    ...
    userMenu.addEventListener('click', function (event) {
      if (event.target.closest('#userUpgradeButton')) return;
    ...
    profileSettingsButton.addEventListener('click', function (event) {
      event.preventDefault();
      closeUserDropdown();
      openProfileSettings();
    });

    profileSettingsClose.addEventListener('click', closeProfileSettings);
    profileSettingsCancelButton.addEventListener('click', closeProfileSettings);
    profileSettingsModal.addEventListener('click', function (event) {
      if (event.target === profileSettingsModal) closeProfileSettings();
    });
    ...
    switchForgotToSignin.addEventListener('click', function () {
      openSiteAuth('signin');
    });
    ...
    siteForgotPasswordForm.addEventListener('submit', async function (event) {
      event.preventDefault();
    ...
    logoutButton.addEventListener('click', async function () {
      ...
      marketUserTools.style.display = 'none';
    ...
    registeredUsersList.addEventListener('click', function (event) {
      ...
      if (resetPasswordButton) {
        adminResetUserPassword(resetPasswordButton.dataset.resetPassword);
      }
    });
    ...
    async function handlePaDownloadSubmit(event) {
      if (event) {
        event.preventDefault();
        event.stopPropagation();
      }
      ...
      downloadTifButton.innerHTML = `
        <span class="download-pa-main">Opening PA...</span>
        <span class="pdf-price-tag">RM5 per unit</span>
      `;

      try {
        window.open(paUrl, '_blank', 'noopener');
        ...
        await recordPurchase(paValue, selectedNegeri, paUrl);

        paStatus.style.display = 'block';
        paStatus.textContent = 'JUPEM PA download opened.';
    ...
    window.azobssHandlePaDownload = handlePaDownloadSubmit;

    paForm.addEventListener('submit', handlePaDownloadSubmit);
    downloadTifButton.addEventListener('click', handlePaDownloadSubmit);
