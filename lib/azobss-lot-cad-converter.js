...
const CONVERTER_VERSION = '1164.1';
...
function dxfLayerR2000(name, color, handle, owner = '1', options = {}) {
  const plotStyleHandle = String(options && options.plotStyleHandle || '').trim();
  const materialHandle = String(options && options.materialHandle || '').trim();
  return dxfPair(0, 'LAYER') +
    ...
    (plotStyleHandle ? dxfPair(390, plotStyleHandle) : '') +
    (materialHandle ? dxfPair(347, materialHandle) : '');
}
...
const AZOBSS_SHEET_LAYER_SPECS = Object.freeze([
  Object.freeze({ name: 'SYIT_PIAWAI', color: 1 }),
  Object.freeze({ name: 'SYIT_LABEL', color: 1 })
]);

function ensureDxfLayers(templateText, layerSpecs = []) {
  const lines = String(templateText || '').replace(/\r\n/g, '\n').split('\n');
  const usedHandles = new Set();
  for (let i = 0; i + 1 < lines.length; i += 2) {
    if (String(lines[i]).trim() === '5') usedHandles.add(String(lines[i + 1]).trim().toUpperCase());
  }
  ...
  for (let i = 0; i + 3 < lines.length; i += 2) {
    if (
      String(lines[i]).trim() === '0' &&
      String(lines[i + 1]).trim() === 'TABLE' &&
      String(lines[i + 2]).trim() === '2' &&
      String(lines[i + 3]).trim() === 'LAYER'
    ) {
      tableStart = i;
      break;
    }
  }
  if (tableStart < 0) throw new Error('Template DXF tidak mempunyai LAYER table.');

  const existingNames = new Set();
  let plotStyleHandle = '';
  let materialHandle = '';
  for (let i = tableStart + 4; i + 1 < lines.length; i += 2) {
    ...
    if (countIndex < 0 && code === '70') countIndex = i;
    if (code === '2' && value && value !== 'LAYER') existingNames.add(value.toUpperCase());
    // v1162: inherit mandatory layer references from the template itself.
    // AC1027 needs PlotStyleName (390) + Material (347); the lean R2000
    // LibreDWG template needs PlotStyleName (390) but has no Material handle.
    if (!plotStyleHandle && code === '390' && value) plotStyleHandle = value;
    if (!materialHandle && code === '347' && value) materialHandle = value;
  }
  if (tableEnd < 0 || countIndex < 0) throw new Error('LAYER table DXF tidak lengkap.');

  const missing = (Array.isArray(layerSpecs) ? layerSpecs : [])
    .filter((spec) => spec && spec.name && !existingNames.has(String(spec.name).toUpperCase()));
  if (!missing.length) return lines.join('\n');

  let nextHandle = 0x900;
  const allocateHandle = () => {
    while (usedHandles.has(nextHandle.toString(16).toUpperCase())) nextHandle += 1;
  ...
  for (const spec of missing) {
    records += dxfLayerR2000(
      String(spec.name),
      Number(spec.color || 7),
      allocateHandle(),
      '1',
      { plotStyleHandle, materialHandle }
    );
  }
  const recordLines = records.replace(/\n$/, '').split('\n');
  lines.splice(tableEnd, 0, ...recordLines);

  const oldCount = Number(String(lines[countIndex + 1]).trim());
  lines[countIndex + 1] = String((Number.isFinite(oldCount) ? oldCount : existingNames.size) + missing.length);
  return lines.join('\n');
}
...
function estimateLibreDwgTextWidth(text, height) {
  // AutoCAD's Standard style is proportional. For the lot/PA labels used here
  // (mostly A-Z and digits), ~0.60 x text height per glyph keeps the visual
  // centre close to AutoCAD's middle-centre TEXT alignment without depending
  // on alignment points that LibreDWG currently relocates on some drawings.
  const value = safeText(text);
  return Math.max(Number(height) * 0.6, value.length * Number(height) * 0.60);
}

function normalizeTextEntitiesForLibreDwg(entitiesText) {
  const lines = String(entitiesText || '').replace(/\r\n/g, '\n').split('\n');
  const out = [];
  let i = 0;

  function pairCode(index) { return index < lines.length ? String(lines[index]).trim() : ''; }
  function pairValue(index) { return index + 1 < lines.length ? String(lines[index + 1]) : ''; }

  while (i + 1 < lines.length) {
    if (pairCode(i) !== '0' || pairValue(i).trim() !== 'TEXT') {
    ...
    const entityPairs = [];
    while (i + 1 < lines.length) {
      if (entityPairs.length && pairCode(i) === '0') break;
      entityPairs.push([pairCode(i), pairValue(i)]);
      i += 2;
    }

    let x10 = NaN, y20 = NaN, x11 = NaN, y21 = NaN;
    for (const [code, value] of entityPairs) {
      if (code === '10' && !Number.isFinite(x10)) x10 = Number(value);
      else if (code === '20' && !Number.isFinite(y20)) y20 = Number(value);
      else if (code === '11' && !Number.isFinite(x11)) x11 = Number(value);
      else if (code === '21' && !Number.isFinite(y21)) y21 = Number(value);
    }

    // v944: use the canonical AutoCAD R2000 representation for TEXT justified
    // Middle Center.  For a non-left TEXT, 72=1 and 73=2 select Center/Middle,
    // and 11/21 is the alignment point.  AutoCAD/ezdxf write 10/20 and 11/21
    // to the same placement point for this mode.  Previous AZOBSS builds used a
    // deliberately offset fallback 10/20 point to work around an older LibreDWG
    // placement bug.  That produced correct-looking text but left two competing
    // anchors in the entity.  With the newer LibreDWG writer v944 uses one
    // canonical centre point, so Properties reports Justify = Middle center and
    // the DWG writer has no artificial anchor discrepancy to repair.
    const cx = Number.isFinite(x11) ? x11 : x10;
    const cy = Number.isFinite(y21) ? y21 : y20;
    const valid = Number.isFinite(cx) && Number.isFinite(cy);

    let have10 = false, have20 = false, have30 = false;
    let have11 = false, have21 = false, have31 = false, have72 = false, have73 = false;
    for (const [code, value] of entityPairs) {
      if (code === '10') { out.push(' 10', valid ? dxfNumber(cx) : value); have10 = true; continue; }
      if (code === '20') { out.push(' 20', valid ? dxfNumber(cy) : value); have20 = true; continue; }
      if (code === '30') { out.push(' 30', '0'); have30 = true; continue; }
      if (code === '72') { out.push(' 72', '1'); have72 = true; continue; }
      if (code === '73') { out.push(' 73', '2'); have73 = true; continue; }
      if (code === '11') { out.push(' 11', valid ? dxfNumber(cx) : value); have11 = true; continue; }
      if (code === '21') { out.push(' 21', valid ? dxfNumber(cy) : value); have21 = true; continue; }
      if (code === '31') { out.push(' 31', '0'); have31 = true; continue; }
      out.push(String(code).padStart(3, ' '), value);
    }

    if (!have10) out.push(' 10', valid ? dxfNumber(cx) : '0');
    if (!have20) out.push(' 20', valid ? dxfNumber(cy) : '0');
    if (!have30) out.push(' 30', '0');
    if (!have72) out.push(' 72', '1');
    if (!have11) out.push(' 11', valid ? dxfNumber(cx) : '0');
    if (!have21) out.push(' 21', valid ? dxfNumber(cy) : '0');
    if (!have31) out.push(' 31', '0');
    if (!have73) out.push(' 73', '2');
  }
  return out.join('\n');
}
...
function buildLibreDwgInputDxf(fullDxfBuffer, report = {}) {
  ...
  const rawEntities = extractEntitiesSectionText(Buffer.isBuffer(fullDxfBuffer) ? fullDxfBuffer.toString('latin1') : String(fullDxfBuffer || ''));
  const entities = normalizeTextEntitiesForLibreDwg(rawEntities);
  const templateText = ensureDxfLayers(fs.readFileSync(templatePath, 'latin1'), AZOBSS_SHEET_LAYER_SPECS);
  let dxf = replaceEntitiesSection(templateText, entities);
...
function buildDxf(features, records, options = {}) {
  ...
  const sheetPrepared = [];
  const sheetSegmentMap = new Map();
  const sheetMinDimensions = [];

  let geometryMinX = Infinity, geometryMinY = Infinity, geometryMaxX = -Infinity, geometryMaxY = -Infinity;
  let sheetMinX = Infinity, sheetMinY = Infinity, sheetMaxX = -Infinity, sheetMaxY = -Infinity;
  ...
  function includeSheetPoint(point) {
    const x = Number(point && point[0]);
    const y = Number(point && point[1]);
    if (!Number.isFinite(x) || !Number.isFinite(y)) return;
    if (x < sheetMinX) sheetMinX = x;
    if (x > sheetMaxX) sheetMaxX = x;
    if (y < sheetMinY) sheetMinY = y;
    if (y > sheetMaxY) sheetMaxY = y;
  }
  ...
  function cleanRing(rawRing) {
    ...
    if (ring.length > 1) {
      const first = ring[0];
      const last = ring[ring.length - 1];
      if (Math.abs(first[0] - last[0]) < 1e-10 && Math.abs(first[1] - last[1]) < 1e-10) ring.pop();
    }
  ...
  for (let i = 0; i < features.length; i += 1) {
    ...
    const rings = (feature.rings || []).map(cleanRing).filter(r => r.length >= 3);
  ...
  const rawSheets = Array.isArray(options.sheetFeatures) ? options.sheetFeatures : [];
  for (let i = 0; i < rawSheets.length; i += 1) {
    const sheet = rawSheets[i] || {};
    const sourceRings = Array.isArray(sheet.rings)
      ? sheet.rings
      : (sheet.geometry && Array.isArray(sheet.geometry.rings) ? sheet.geometry.rings : []);
    const rings = sourceRings.map(cleanRing).filter(r => r.length >= 3);
    if (!rings.length) continue;
    ...
    for (const ring of rings.slice(1)) {
      ...
      if (area > maxArea) { maxArea = area; labelRing = ring; }
    }

    const attrs = sheet.attributes && typeof sheet.attributes === 'object' ? sheet.attributes : {};
    const name = safeText(
      sheet.name || sheet.sheetName || sheet.noSyit || attrs.NAMA || attrs.NAME ||
      attrs.NO_SYIT || attrs.NOSYIT || attrs.SYIT || attrs.LEMBAR || attrs.PIAWAI || ('Syit ' + (i + 1))
    );
    ...
    for (const ring of rings) {
      for (let p = 0; p < ring.length; p += 1) {
        ...
        const b = ring[(p + 1) % ring.length];
        if (Math.abs(a[0] - b[0]) < 1e-10 && Math.abs(a[1] - b[1]) < 1e-10) continue;
        includeSheetPoint(a); includeSheetPoint(b);
        const key = segmentKey(a, b);
        if (!sheetSegmentMap.has(key)) sheetSegmentMap.set(key, [a, b]);
      }
    }

    const bounds = ringBounds(labelRing);
    const minDimension = Math.max(0, Math.min(Math.abs(bounds.width), Math.abs(bounds.height)));
    if (minDimension > 0) sheetMinDimensions.push(minDimension);
    sheetPrepared.push({ name, labelRing, bounds });
  }

  if (sheetSegmentMap.size) {
    const overlapsLotBounds = !(
      sheetMaxX < geometryMinX || sheetMinX > geometryMaxX ||
      sheetMaxY < geometryMinY || sheetMinY > geometryMaxY
    );
    if (!overlapsLotBounds) {
      throw new Error('Garisan Syit Piawai tidak sepadan dengan sistem koordinat Lot Kadaster. DXF tidak dijana untuk mengelakkan garisan tersasar.');
    }
  }

  const medianArea = median(areas);
  const configuredHeight = Number(options.textHeight || process.env.AZOBSS_LOT_CAD_TEXT_HEIGHT || 0);
  const textHeight = configuredHeight > 0
    ? configuredHeight
    : Math.max(0.65, Math.min(2.8, Math.sqrt(Math.max(1, medianArea)) * 0.12));

  const medianSheetDimension = median(sheetMinDimensions);
  const sheetTextHeight = sheetPrepared.length
    ? Math.max(textHeight * 2.5, Math.min(45, Math.max(textHeight * 2.5, medianSheetDimension * 0.018)))
    : 0;

  // Draw the Syit Piawai grid first, then the cadastral lot detail on top.
  const handles = createDxfHandleAllocator(0x1000);
  let entities = '';

  for (const pair of sheetSegmentMap.values()) {
    entities += dxfLineR2000('SYIT_PIAWAI', pair[0], pair[1], handles.next());
  }

  let sheetTextCount = 0;
  for (const sheet of sheetPrepared) {
    if (!sheet.name) continue;
    const point = interiorLabelPoint(sheet.labelRing);
    entities += dxfTextR2000('SYIT_LABEL', sheet.name, point[0], point[1], sheetTextHeight, handles.next());
    sheetTextCount += 1;
  }

  for (const pair of segmentMap.values()) {
    entities += dxfLineR2000('PER NDCDB', pair[0], pair[1], handles.next());
  }
  ...
  // Header extents include every full relevant sheet. The initial viewport
  // remains focused on the purchased lot area.
  const drawingMinX = sheetSegmentMap.size ? Math.min(geometryMinX, sheetMinX) : geometryMinX;
  const drawingMinY = sheetSegmentMap.size ? Math.min(geometryMinY, sheetMinY) : geometryMinY;
  const drawingMaxX = sheetSegmentMap.size ? Math.max(geometryMaxX, sheetMaxX) : geometryMaxX;
  const drawingMaxY = sheetSegmentMap.size ? Math.max(geometryMaxY, sheetMaxY) : geometryMaxY;

  const drawingRawWidth = Math.max(1, drawingMaxX - drawingMinX);
  const drawingRawHeight = Math.max(1, drawingMaxY - drawingMinY);
  const drawingMargin = Math.max(
    textHeight * 10,
    sheetTextHeight > 0 ? sheetTextHeight * 1.5 : 0,
    Math.max(drawingRawWidth, drawingRawHeight) * 0.02,
    5
  );
  const minX = drawingMinX - drawingMargin;
  const minY = drawingMinY - drawingMargin;
  const maxX = drawingMaxX + drawingMargin;
  const maxY = drawingMaxY + drawingMargin;

  const lotRawWidth = Math.max(1, geometryMaxX - geometryMinX);
  const lotRawHeight = Math.max(1, geometryMaxY - geometryMinY);
  const lotViewMargin = Math.max(textHeight * 10, Math.max(lotRawWidth, lotRawHeight) * 0.035, 5);
  const viewMinX = geometryMinX - lotViewMargin;
  const viewMinY = geometryMinY - lotViewMargin;
  const viewMaxX = geometryMaxX + lotViewMargin;
  const viewMaxY = geometryMaxY + lotViewMargin;
  const centerX = (viewMinX + viewMaxX) / 2;
  const centerY = (viewMinY + viewMaxY) / 2;
  const aspect = 16 / 9;
  const viewHeight = Math.max(viewMaxY - viewMinY, (viewMaxX - viewMinX) / aspect) * 1.02;
  ...
  return {
    ...
    report: {
      ...
      lineEntities: segmentMap.size + sheetSegmentMap.size,
      lotLineEntities: segmentMap.size,
      sheetLineEntities: sheetSegmentMap.size,
      sheetLabelCount: sheetTextCount,
      sheetNames: sheetPrepared.map((sheet) => sheet.name).filter(Boolean),
      ...
      sheetGeometry: sheetSegmentMap.size
        ? 'full relevant sheet boundaries; not clipped to purchased AOI'
        : 'not available for this legacy purchase',
      ...
      autoView: 'drawing opens centred on purchased lot extents; ZOOM Extents shows full sheet boundaries'
    }
  };
}
...
function findDwgRewrite(root) {
  const candidates = [
    process.env.AZOBSS_DWGREWRITE_PATH,
    root && path.join(root, '.azobss-libredwg', 'bin', 'dwgrewrite'),
  ...
  if (commandExists('dwgrewrite')) return 'dwgrewrite';
  return '';
}
...
function ensureDwgConverterSync(root, options = {}) {
  ...
  const existingRewrite = findDwgRewrite(resolvedRoot);
  if (existing && existingRewrite) return { ok: true, available: true, executable: existing, rewriteExecutable: existingRewrite, alreadyAvailable: true };
  ...
  if (dwgInstallAttempted && !options.force) {
    ...
    const nowRewrite = findDwgRewrite(resolvedRoot);
    return Object.assign({ ok: !!now && !!nowRewrite, available: !!now && !!nowRewrite, executable: now || '', rewriteExecutable: nowRewrite || '' }, lastDwgInstallStatus || {});
  }
  ...
  const rewriteExecutable = findDwgRewrite(resolvedRoot);
  const detail = String((result && (result.stderr || result.stdout)) || '')
    .replace(/\s+/g, ' ').trim().slice(-1600);
  lastDwgInstallStatus = {
    ok: !!executable && !!rewriteExecutable,
    available: !!executable && !!rewriteExecutable,
    executable: executable || '',
    rewriteExecutable: rewriteExecutable || '',
    exitCode: result && typeof result.status === 'number' ? result.status : null,
    timedOut: !!(result && result.error && result.error.code === 'ETIMEDOUT'),
    error: (executable && rewriteExecutable) ? '' : (detail || String(result && result.error && result.error.message || 'LibreDWG install gagal.'))
  };
  return lastDwgInstallStatus;
}
...
function chooseShapefileEntries(entries) {
  ...
  const prj = entries.find(entry => entry.name.toLowerCase() === `${base}.prj`.toLowerCase()) ||
    entries.find(entry => /\.prj$/i.test(entry.name) && path.basename(entry.name, path.extname(entry.name)).toLowerCase() === path.basename(base).toLowerCase());
  if (!dbf) throw new Error('Fail .DBF pasangan tidak ditemui dalam ZIP Lot Kadaster.');
  return { shp, dbf, prj: prj || null };
}
...
function parseLotZip(zipBuffer, tempDir) {
  ...
  const projectionWkt = chosen.prj
    ? readZipEntry(zipBuffer, chosen.prj).toString('utf8').replace(/\0/g, '').trim()
    : '';
  ...
  return {
    ...
    entries: {
      shp: chosen.shp.name,
      dbf: chosen.dbf.name,
      prj: chosen.prj ? chosen.prj.name : ''
    }
  };
}


function lotFeatureBounds(features) {
  let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
  for (const feature of Array.isArray(features) ? features : []) {
    for (const ring of feature && Array.isArray(feature.rings) ? feature.rings : []) {
...
function inspectLotZipBounds(zipBuffer) {
  const parsed = parseLotZip(zipBuffer);
  const bounds = lotFeatureBounds(parsed.features);
  if (!bounds) throw new Error('Koordinat SHP Lot Kadaster tidak dapat dibaca untuk padanan sistem koordinat.');
  return {
    bounds,
    featureCount: parsed.features.length,
    entries: parsed.entries,
    projectionWkt: String(parsed.projectionWkt || '').trim()
  };
}
...
function convertDxfToDwg(dxfBuffer, options = {}) {
  ...
  const cleanDwgPath = makeTempPath(tempDir, '.clean.dwg');
  try {
    ...
    const rawSignature = rawBuffer.subarray(0, 6).toString('ascii');
    if (rawBuffer.length < 128 || !/^AC10\d\d$/.test(rawSignature)) throw new Error('DWG yang dijana tidak mempunyai signature AutoCAD yang sah.');

    // dxf2dwg can produce a readable DWG that AutoCAD still flags for RECOVER.
    // Canonicalize it through LibreDWG's own read -> write -> re-read utility.
    // dwgrewrite exits non-zero on critical read/write/re-read failures, so we do
    // not serve a DWG that failed the sanitation pass.
    const rewriteExecutable = findDwgRewrite(root);
    if (!rewriteExecutable) {
      const error = new Error('DWG sanitation utility belum tersedia. Gunakan DXF sementara atau redeploy backend Docker v942.');
      error.code = 'DWG_REWRITE_UNAVAILABLE';
      throw error;
    }
    const rewrite = childProcess.spawnSync(rewriteExecutable, ['-v0', '--as', 'r2000', dwgPath, cleanDwgPath], {
      cwd: tempDir,
      encoding: 'utf8',
      timeout: 120000,
      maxBuffer: 16 * 1024 * 1024,
    ...
    if (rewrite.error) throw rewrite.error;
    if (rewrite.status !== 0 || !fs.existsSync(cleanDwgPath)) {
      const detail = String(rewrite.stderr || rewrite.stdout || '').replace(/\s+/g, ' ').trim().slice(0, 800);
      throw new Error('DWG sanitation/rewrite gagal' + (detail ? `: ${detail}` : '.'));
    }
    const buffer = fs.readFileSync(cleanDwgPath);
    const signature = buffer.subarray(0, 6).toString('ascii');
    if (buffer.length < 128 || signature !== 'AC1015') throw new Error('DWG selepas rewrite R2000 tidak mempunyai signature AC1015 yang sah.');
    return { buffer, executable, rewriteExecutable, signature, rawSignature, rewriteApplied: true };
  } finally {
    try { if (fs.existsSync(dxfPath)) fs.unlinkSync(dxfPath); } catch (_) {}
    try { if (fs.existsSync(dwgPath)) fs.unlinkSync(dwgPath); } catch (_) {}
    try { if (fs.existsSync(cleanDwgPath)) fs.unlinkSync(cleanDwgPath); } catch (_) {}
  }
}
...
function convertLotZip(zipBuffer, format, options = {}) {
  ...
  return { format: 'dwg', buffer: dwg.buffer, report: Object.assign({}, dxf.report, { dwgSignature: dwg.signature, dwgInputProfile: 'AC1015 lean LibreDWG profile + canonical Middle Center TEXT', dwgSanitizer: 'dwgrewrite-r2000', dwgTextJustification: 'Middle Center', dwgOutputVersion: 'AutoCAD 2000 / AC1015', dwgRewriteApplied: !!dwg.rewriteApplied }) };
}

function dwgAvailable(root) { const resolved = root || process.cwd(); return !!findDxf2Dwg(resolved) && !!findDwgRewrite(resolved); }

module.exports = { CONVERTER_VERSION, convertLotZip, buildDxf, buildLibreDwgInputDxf, parseShp, parseDbf, inspectLotZipBounds, dwgAvailable, findDxf2Dwg, findDwgRewrite, ensureDwgConverterSync };
