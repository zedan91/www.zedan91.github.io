...
function convertDxfToDwg(dxfBuffer, options = {}) {
  ...
  if (!executable) {
    ...
    if (!executable) {
      ...
      const error = new Error('DWG converter belum aktif. Pastikan backend v936 berjalan dalam Docker runtime supaya LibreDWG dibina semasa deploy; DXF masih boleh dimuat turun.' + detail);
