...
function buildDxf(features, records, options = {}) {
  ...
  // AC1015 / AutoCAD 2000 is intentional. R12 (AC1009) does not reliably
  // accept spaces in symbol-table names on older AutoCAD-compatible programs,
  // which caused "Improper table entry name PER NDCDB" and an empty drawing.
  // R2000 preserves the exact requested layer name "PER NDCDB".
  const handles = createDxfHandleAllocator(0x100);
  let entities = '';
  ...
  dxf += dxfPair(9, '$DWGCODEPAGE') + dxfPair(3, 'ANSI_1252');
  dxf += dxfPair(9, '$INSUNITS') + dxfPair(70, 6); // metres
  dxf += dxfPair(9, '$MEASUREMENT') + dxfPair(70, 1); // metric
  dxf += dxfPair(9, '$TEXTSIZE') + dxfPair(40, dxfNumber(textHeight));
  dxf += dxfPair(9, '$HANDSEED') + dxfPair(5, handles.handseed());
  dxf += dxfPair(0, 'ENDSEC');

  dxf += dxfPair(0, 'SECTION') + dxfPair(2, 'TABLES');

  dxf += dxfPair(0, 'TABLE') + dxfPair(2, 'LTYPE') + dxfPair(5, '2') + dxfPair(330, '0') + dxfPair(100, 'AcDbSymbolTable') + dxfPair(70, 1);
  dxf += dxfPair(0, 'LTYPE') + dxfPair(5, '20') + dxfPair(330, '2') +
    dxfPair(100, 'AcDbSymbolTableRecord') + dxfPair(100, 'AcDbLinetypeTableRecord') +
    dxfPair(2, 'Continuous') + dxfPair(70, 0) + dxfPair(3, 'Solid line') +
    dxfPair(72, 65) + dxfPair(73, 0) + dxfPair(40, 0);
  dxf += dxfPair(0, 'ENDTAB');

  dxf += dxfPair(0, 'TABLE') + dxfPair(2, 'LAYER') + dxfPair(5, '1') + dxfPair(330, '0') + dxfPair(100, 'AcDbSymbolTable') + dxfPair(70, 4);
  dxf += dxfLayerR2000('0', 7, '21');
  dxf += dxfLayerR2000('PER NDCDB', 7, '22');
  dxf += dxfLayerR2000('NOLOT', 7, '23');
  dxf += dxfLayerR2000('NOPA', 2, '24');
  dxf += dxfPair(0, 'ENDTAB');

  dxf += dxfPair(0, 'TABLE') + dxfPair(2, 'STYLE') + dxfPair(5, '5') + dxfPair(330, '0') + dxfPair(100, 'AcDbSymbolTable') + dxfPair(70, 1);
  dxf += dxfPair(0, 'STYLE') + dxfPair(5, '25') + dxfPair(330, '5') +
    dxfPair(100, 'AcDbSymbolTableRecord') + dxfPair(100, 'AcDbTextStyleTableRecord') +
    dxfPair(2, 'Standard') + dxfPair(70, 0) + dxfPair(40, 0) + dxfPair(41, 1) +
    dxfPair(50, 0) + dxfPair(71, 0) + dxfPair(42, 2.5) + dxfPair(3, 'txt') + dxfPair(4, '');
  dxf += dxfPair(0, 'ENDTAB');

  dxf += dxfPair(0, 'TABLE') + dxfPair(2, 'BLOCK_RECORD') + dxfPair(5, '9') + dxfPair(330, '0') + dxfPair(100, 'AcDbSymbolTable') + dxfPair(70, 2);
  dxf += dxfPair(0, 'BLOCK_RECORD') + dxfPair(5, '17') + dxfPair(330, '9') + dxfPair(100, 'AcDbSymbolTableRecord') + dxfPair(100, 'AcDbBlockTableRecord') + dxfPair(2, '*Model_Space');
  dxf += dxfPair(0, 'BLOCK_RECORD') + dxfPair(5, '1B') + dxfPair(330, '9') + dxfPair(100, 'AcDbSymbolTableRecord') + dxfPair(100, 'AcDbBlockTableRecord') + dxfPair(2, '*Paper_Space');
  dxf += dxfPair(0, 'ENDTAB');
  dxf += dxfPair(0, 'ENDSEC');

  dxf += dxfPair(0, 'SECTION') + dxfPair(2, 'BLOCKS');
  for (const block of [
    { owner: '17', begin: '18', end: '19', name: '*Model_Space' },
    { owner: '1B', begin: '1C', end: '1D', name: '*Paper_Space' }
  ]) {
    dxf += dxfPair(0, 'BLOCK') + dxfPair(5, block.begin) + dxfPair(330, block.owner) +
      dxfPair(100, 'AcDbEntity') + dxfPair(8, '0') + dxfPair(100, 'AcDbBlockBegin') +
      dxfPair(2, block.name) + dxfPair(70, 0) + dxfPair(10, 0) + dxfPair(20, 0) + dxfPair(30, 0) +
      dxfPair(3, block.name) + dxfPair(1, '');
    dxf += dxfPair(0, 'ENDBLK') + dxfPair(5, block.end) + dxfPair(330, block.owner) +
  ...
  return {
    ...
    report: {
      ...
      dxfVersion: 'AutoCAD 2000 / AC1015',
      units: 'metres',
...
function convertDxfToDwg(dxfBuffer, options = {}) {
  ...
  if (!executable) {
    ...
    if (!executable) {
      ...
      const error = new Error('DWG converter belum berjaya dipasang pada Render. Semak build/runtime log LibreDWG; DXF masih boleh dimuat turun.' + detail);
...
module.exports = { CONVERTER_VERSION, convertLotZip, buildDxf, parseShp, parseDbf, dwgAvailable, findDxf2Dwg, ensureDwgConverterSync };
