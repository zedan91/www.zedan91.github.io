...
function normalizeTextEntitiesForLibreDwg(entitiesText) {
  ...
  while (i + 1 < lines.length) {
    ...
    let x = NaN, y = NaN, h = NaN, text = '';
    for (const [code, value] of entityPairs) {
      if (code === '10' && !Number.isFinite(x)) x = Number(value);
      else if (code === '20' && !Number.isFinite(y)) y = Number(value);
      else if (code === '40' && !Number.isFinite(h)) h = Number(value);
      else if (code === '1' && !text) text = value;
    }

    // v943: keep the final AutoCAD TEXT property as Justify = Middle Center.
    // A non-left TEXT has two points: 10/20 is its insertion/fallback point,
    // while 11/21 is the true alignment point used with 72/73. Earlier builds
    // sent the same centre coordinate in both points and LibreDWG could relocate
    // aligned labels. v940 avoided that by flattening to Left/Baseline, but the
    // resulting DWG no longer reported Middle Center in Properties.
    //
    // Use a dual-anchor representation instead: 11/21 remains the exact lot
    // centre and 72=1 / 73=2 explicitly requests Center/Middle. 10/20 is moved
    // to a sensible lower-left fallback location. This keeps the label visually
    // stable even if a converter consults the insertion point, while AutoCAD sees
    // the entity as genuine Middle Center TEXT.
    const valid = Number.isFinite(x) && Number.isFinite(y) && Number.isFinite(h) && h > 0;
    const insertX = valid ? x - (estimateLibreDwgTextWidth(text, h) / 2) : x;
    const insertY = valid ? y - (h / 2) : y;

    let have11 = false, have21 = false, have31 = false, have72 = false, have73 = false;
    for (const [code, value] of entityPairs) {
      if (code === '10' && valid) { out.push(' 10', dxfNumber(insertX)); continue; }
      if (code === '20' && valid) { out.push(' 20', dxfNumber(insertY)); continue; }
      if (code === '72') { out.push(' 72', '1'); have72 = true; continue; }
      if (code === '73') { out.push(' 73', '2'); have73 = true; continue; }
      if (code === '11') { out.push(' 11', valid ? dxfNumber(x) : value); have11 = true; continue; }
      if (code === '21') { out.push(' 21', valid ? dxfNumber(y) : value); have21 = true; continue; }
      if (code === '31') { out.push(' 31', '0'); have31 = true; continue; }
      out.push(String(code).padStart(3, ' '), value);
    }

    // dxfTextR2000 already emits these pairs, but keep this defensive fallback
    // so future entity writers cannot silently drop the Middle Center metadata.
    if (!have72) out.push(' 72', '1');
    if (!have11) out.push(' 11', valid ? dxfNumber(x) : '0');
    if (!have21) out.push(' 21', valid ? dxfNumber(y) : '0');
...
function convertDxfToDwg(dxfBuffer, options = {}) {
  ...
  try {
    ...
    const rewrite = childProcess.spawnSync(rewriteExecutable, ['-v0', '--as', 'r14', dwgPath, cleanDwgPath], {
      cwd: tempDir,
      encoding: 'utf8',
      timeout: 120000,
      maxBuffer: 16 * 1024 * 1024,
    ...
    if (buffer.length < 128 || signature !== 'AC1014') throw new Error('DWG selepas rewrite R14 tidak mempunyai signature AC1014 yang sah.');
