...
function normalizeTextEntitiesForLibreDwg(entitiesText) {
  ...
  while (i + 1 < lines.length) {
    ...
    // The public AC1027 DXF uses middle-centre TEXT (72=1, 73=2) with group
    // 11/21 as the alignment point. LibreDWG 0.14.x can preserve the text
    // string but misplace these aligned entities in the produced DWG, causing
    // labels to collect in a remote block. For the internal DWG-only DXF,
    // flatten each label to plain left/baseline TEXT and pre-offset its insert
    // point so its visual centre remains at the same coordinate.
    ...
    let emittedSecondTextSubclass = false;
    for (const [code, value] of entityPairs) {
      if (code === '10' && valid) { out.push(' 10', dxfNumber(insertX)); continue; }
      if (code === '20' && valid) { out.push(' 20', dxfNumber(insertY)); continue; }
      if (code === '72') { out.push(' 72', '0'); continue; }
      if (code === '11' || code === '21' || code === '31') continue;
      if (code === '73') { out.push(' 73', '0'); continue; }
      if (code === '100' && String(value).trim() === 'AcDbText') {
        if (emittedSecondTextSubclass) { out.push('100', 'AcDbText'); }
        else { out.push('100', 'AcDbText'); emittedSecondTextSubclass = true; }
        continue;
      }
...
function convertDxfToDwg(dxfBuffer, options = {}) {
  ...
  try {
    ...
    if (buffer.length < 128 || !/^AC10\d\d$/.test(signature)) throw new Error('DWG selepas rewrite tidak mempunyai signature AutoCAD yang sah.');
...
function convertLotZip(zipBuffer, format, options = {}) {
  ...
  return { format: 'dwg', buffer: dwg.buffer, report: Object.assign({}, dxf.report, { dwgSignature: dwg.signature, dwgInputProfile: 'AC1015 lean LibreDWG profile + flattened TEXT alignment', dwgSanitizer: 'dwgrewrite-r2000', dwgRewriteApplied: !!dwg.rewriteApplied }) };
}
