...
function azBuildAdminPaBmTestCheckout(data = {}, identity = {}) {
  ...
  for (const rawItem of rawItems) {
    ...
    let baseAmount = 0;
    if (productType === "PA") baseAmount = 5;
    else if (productType === "BM" || productType === "SBM") baseAmount = 3;
    else if (productType === "GPS") baseAmount = 9;
    else if (productType === "SYIT_PIAWAI") baseAmount = 7;
    else if (areaProductTypes.has(productType)) baseAmount = verifiedLot.amount;
    ...
    const amount = azApplyUserPriceAdjustment(baseAmount, identity, priceAdjustmentCategory);
...
function azobssJupemParseHtmlAttributes(tagSource) {
  const attrs = {};
...
function azobssJupemExtractForms(html) {
  const source = String(html || "");
  const forms = [];
  const formPattern = /<form\b([^>]*)>([\s\S]*?)<\/form>/gi;
  let formMatch;

  while ((formMatch = formPattern.exec(source))) {
    const formAttrs = azobssJupemParseHtmlAttributes(formMatch[1]);
    ...
    let inputMatch;
    while ((inputMatch = inputPattern.exec(inner))) {
      const attrs = azobssJupemParseHtmlAttributes(inputMatch[1]);
      ...
      if (["checkbox", "radio"].includes(type) && !/\bchecked(?:\s|=|>|$)/i.test(inputMatch[0])) continue;
    ...
    let buttonMatch;
    while ((buttonMatch = buttonPattern.exec(inner))) {
      const attrs = azobssJupemParseHtmlAttributes(buttonMatch[1]);
      const name = String(attrs.name || "").trim();
      const value = String(attrs.value || "");
      const buttonText = decodeHtmlEntities(String(buttonMatch[2] || "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim());
      submitters.push({
        name,
        value,
        text: buttonText,
    ...
    forms.push({
      action: decodeHtmlEntities(String(formAttrs.action || "")),
      method: String(formAttrs.method || "POST").toUpperCase(),
      html: inner,
      fields,
      submitters
    });
...
function azobssJupemBuildFormBody(form, overrides = {}) {
  const body = new URLSearchParams();
  for (const field of (form && form.fields) || []) {
    if (!field || !field.name) continue;
    body.append(String(field.name), String(field.value ?? ""));
  }
  for (const [name, value] of Object.entries(overrides || {})) {
    if (value === undefined || value === null) continue;
    body.set(String(name), String(value));
  }
  return body;
}

function azobssJupemFindPasswordForm(html) {
  return azobssJupemExtractForms(html).find((form) =>
    /name\s*=\s*["']KataLaluan["']/i.test(form.html)
    || form.fields.some((field) => /^KataLaluan$/i.test(field.name))
  ) || null;
}
...
function azobssJupemFindSecurityPhraseConfirmForm(html) {
  ...
  for (const form of forms) {
    const formText = decodeHtmlEntities(String(form.html || "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim());
    const hasPhrase = /FrasaRahsia|frasa\s+rahsia|imej\s+keselamatan|security\s+(?:phrase|image)/i.test(form.html + " " + formText);
    if (!hasPhrase) continue;

    const submitter = (form.submitters || []).find((item) => {
      const value = String(item && item.value || "").trim();
      const label = String(item && item.text || "").trim();
      return affirmative.test(value) || affirmative.test(label);
    });

    if (submitter) return { form, submitter };
  }
  return null;
}

async function azobssJupemResolvePasswordStage(secondHtml, secondUrl, secondCookie, username) {
  let html = String(secondHtml || "");
  ...
  const confirmation = azobssJupemFindSecurityPhraseConfirmForm(html);
  if (!confirmation) {
    if (azobssJupemLoginFailureLooksCredentialRelated(html)) {
      throw Object.assign(new Error("JUPEM eBiz menolak ID pengguna. Sila semak akaun JUPEM eBiz di Render."), { permanent: true });
    }
    throw new Error("JUPEM memaparkan peringkat log masuk yang tidak dikenali selepas ID pengguna.");
  }

  const { form, submitter } = confirmation;
  const overrides = {};
  const idField = (form.fields || []).find((field) => /^IDPengguna$/i.test(field.name));
  if (idField) overrides[idField.name] = idField.value || username;
  else overrides.IDPengguna = username;

  if (submitter && submitter.name) {
    overrides[submitter.name] = submitter.value || submitter.text || "Ya";
  }

  const confirmAction = submitter && submitter.formaction
    ? submitter.formaction
    : (form.action || url);

  const confirmed = await azobssJupemAuthFetch(confirmAction, {
    method: form.method || "POST",
    contentType: "application/x-www-form-urlencoded",
    referer: url,
    body: azobssJupemBuildFormBody(form, overrides).toString(),
    timeoutMs: 30000
  }, cookie);
  ...
  if (!passwordForm) {
    if (azobssJupemLoginFailureLooksCredentialRelated(html) || azobssJupemIsLoginPage(html, url)) {
      throw Object.assign(new Error("JUPEM eBiz tidak meneruskan ke peringkat kata laluan selepas pengesahan frasa keselamatan."), { permanent: false });
    }
    throw new Error("Borang kata laluan JUPEM tidak ditemui selepas pengesahan frasa keselamatan.");
  }

  return { html, url, cookie, passwordForm, usedPhraseConfirmation: true };
}
...
async function azobssResolveLotGpNaturalInput(estimate, auth) {
  const cacheKey = `${estimate.config.gp}|${estimate.config.lotLayer}`;
  const cached = azobssLotGpClipLayerCache.get(cacheKey);
  let dataType = cached && cached.expiresAt > Date.now() && cached.dataType
    ? String(cached.dataType)
  ...
  if (!(cached && cached.expiresAt > Date.now() && cached.dataType)) {
    try {
    ...
    azobssLotGpClipLayerCache.set(cacheKey, { dataType, expiresAt: Date.now() + (30 * 60 * 1000) });
  }

  const fieldCandidate = String(estimate && estimate.featureSet && estimate.featureSet.objectIdFieldName || "OBJECTID");
  const objectIdFieldName = /^[A-Za-z_][A-Za-z0-9_]*$/.test(fieldCandidate) ? fieldCandidate : "OBJECTID";
  const objectIds = Array.isArray(estimate && estimate.selectedObjectIds)
    ? estimate.selectedObjectIds.map((value) => String(value).trim()).filter((value) => /^-?\d+$/.test(value))
    : [];
  if (!objectIds.length) throw new Error("ID lot terpilih JUPEM tidak tersedia untuk eksport natural.");

  // ArcGIS GPFeatureRecordSetLayer accepts a layer URL plus an attribute filter.
  // This keeps the server-side input compact while restricting the export to only
  // the lots that intersected the user's reference area.
  const layerReference = {
    url: layerUrl,
    filter: `${objectIdFieldName} IN (${objectIds.join(",")})`
  };
  return {
    dataType,
    layerUrl,
    objectIdFieldName,
    selectedObjectIdCount: objectIds.length,
    value: azobssLotGpFeatureSetValueForType(layerReference, dataType)
  };
}
      ...
      for (const candidate of candidates) {
        try {
          ...
          return {
            ...
            clipLayers: [naturalInput.layerUrl],
          ...
          console.warn(`JUPEM natural lot export input ${candidate.kind} failed:`, error && (error.message || error));
        }
      }
      throw candidateError || new Error("JUPEM natural lot export input was rejected.");
    } catch (error) {
      lastError = error;
...
async function azobssEnsureLotCadBuffer(record, type, format) {
  ...
  if (normalizedFormat === "dwg" && (!azobssLotCadConverter.dwgAvailable || !azobssLotCadConverter.dwgAvailable(ROOT))) {
    ...
    if (!azobssLotCadConverter.dwgAvailable || !azobssLotCadConverter.dwgAvailable(ROOT)) {
      ...
      const error = new Error("DWG converter belum aktif pada backend. v938 menggunakan Render Docker runtime supaya LibreDWG dibina dan diuji semasa deploy. DXF masih boleh dimuat turun. Kuota download tidak digunakan." + (detail ? ` Detail: ${detail}` : ""));
...
async function azCreatePublicServiceBooking(req, body = {}) {
  ...
  const row = {
    ...
    extraRequests:azServiceBookingText(body.extraRequests, 70),
...
async function handler(req, res) {
  ...
  try {
    ...
    if (pathname === "/api/admin/test-pa-bm-payment" && req.method === "POST") {
      ...
      try {
        let adminIdentity = await azAdminIdentityFromRequest(req, parsed);
        if (!adminIdentity || !adminIdentity.isAdmin) {
          return send(res, 403, JSON.stringify({ ok:false, success:false, error:"Admin authorization required." }, null, 2), "application/json");
        }
        const submittedUser = getPremiumUser(data);
        adminIdentity = await azHydrateIdentityFromUsernameHint(adminIdentity, data.usernameKey || submittedUser.username, data.priceProfileDocId);
        // Preserve the authorization result from the trusted admin allow-list after
        // hydrating only the pricing/profile fields from the verified users document.
        ...
        let order = upsertPremiumOrder({
          ...
          saleAmountText:azAdjustedMoneyText(checkout.totalAmount),
    ...
    if (pathname === "/api/toyyib/create-pa-bm-bill" && req.method === "POST") {
      ...
      try {
        ...
        let identity = await azCommissionIdentityFromRequest(req);
      ...
      try {
        ...
        if (azobssIsTransientJupemError(error)) {
          ...
          return send(res, 202, JSON.stringify({
            ...
            exactBoundaryClip: false,
            naturalLotGeometry: Boolean(retryPayload && retryPayload.naturalLotGeometry),
...
if (pathname === "/api/pa-bm-download" && req.method === "GET") {
  ...
  if (prepareOnly) {
    ...
    if (requestedLotFormat === "dwg" && (!azobssLotCadConverter.dwgAvailable || !azobssLotCadConverter.dwgAvailable(ROOT))) {
      ...
      if (!azobssLotCadConverter.dwgAvailable || !azobssLotCadConverter.dwgAvailable(ROOT)) {
        const detail = String(prep && prep.error || "").slice(0, 700);
        return azobssPaBmDownloadError(res, 503, "DWG converter belum aktif pada backend. Pastikan service azobss-backend menggunakan Docker runtime v938 dan redeploy. Gunakan DXF sementara jika deployment Docker belum selesai." + (detail ? ` Detail: ${detail}` : ""));
      }
    }
