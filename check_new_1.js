...
(function(){
  ...
  function affiliateRule(title){
    ...
    if(has(/tefal|supercook|cookware|frypan|frying pan|wok|stewpot|soup pot|casserole|saucepan|pan\b|\bpot\b|spatula|ladle|knife|chopper|cutlery|kitchen|dapur|periuk|kuali|penggoreng|senduk|pinggan|mangkuk|bekas makanan|food container|lunch box|tupperware|thermos|flask/)){
      return {category:'home-living', badge:'Kitchen Essentials', icon:'ðŸ³', meta:'Best for cooking and daily kitchen use', desc:'sesuai untuk kegunaan dapur harian. Praktikal untuk memasak, penyediaan makanan dan kegunaan rumah.'};
    }
    ...
    if(has(/air fryer|rice cooker|pressure cooker|multi cooker|slow cooker|induction cooker|microwave|oven|toaster|blender|mixer|food processor|meat grinder|grinder|kettle|water dispenser|coffee maker|juicer|appliance|mesin basuh|washing machine|refrigerator|fridge|freezer|iron|steam iron/)){
      return {category:'home-appliances', badge:'Home Appliance', icon:'ðŸ”Œ', meta:'Best for easier daily home use', desc:'sesuai untuk memudahkan kerja harian di rumah. Berguna untuk menjimatkan masa dan lebih praktikal digunakan setiap hari.'};
    }
    ...
    if(has(/vacuum|cordless vacuum|handheld vacuum|cleaner|mop|spin mop|robot vacuum|penyapu|habuk|dust|lint remover|washer|cleaning|pembersih|detergent|sabun lantai|trash bin|tong sampah/)){
      return {category:'home-living', badge:'Cleaning Gadget', icon:'ðŸ§¹', meta:'Best for home and car cleaning', desc:'sesuai untuk pembersihan rumah atau kereta. Mudah digunakan untuk habuk, celah kecil dan kegunaan harian.'};
    }
    ...
    if(has(/ssd|nvme|m\.2|hard disk|hdd|ram|ddr4|ddr5|gpu|rtx|gtx|radeon|processor|cpu|motherboard|pc case|power supply|psu|monitor|keyboard|mouse|gaming mouse|printer|scanner|webcam|usb hub|type-c hub|laptop stand|thermal paste|cooling fan|cooler|speaker|headset|earphone|microphone|mic|capture card/)){
      return {category:'computer', badge: has(/ssd|nvme|m\.2|storage/) ? 'Fast Storage' : 'Computer & Accessories', icon:'ðŸ’»', meta: has(/ssd|nvme|m\.2/) ? 'Best for Windows and game loading' : 'Best for PC setup and daily use', desc:'sesuai untuk setup PC, laptop atau kerja harian. Berguna untuk upgrade, produktiviti dan penggunaan digital.'};
    }
    ...
    if(has(/iphone|android|smartphone|phone|telefon|phone case|casing|screen protector|tempered glass|charger|fast charger|powerbank|power bank|usb c|type c|lightning cable|cable|adapter|magsafe|holder phone|phone holder|tripod phone/)){
      return {category:'mobile', badge:'Daily Tech', icon:'ðŸ“±', meta:'Useful mobile gadget for daily use', desc:'sesuai untuk kegunaan telefon harian. Praktikal untuk caj, perlindungan, aksesori dan penggunaan mudah alih.'};
    }
    ...
    if(has(/dashcam|dash cam|car camera|car vacuum|car charger|jump starter|tyre|tire|tayar|automotive|kereta|motor|motorcycle|car mat|carpet car|seat cover|car holder|windshield|wiper|engine oil|minyak hitam|polish|wax|coating/)){
      return {category:'automotive', badge:'Car Essential', icon:'ðŸš—', meta:'Best for daily car use', desc:'sesuai untuk kegunaan kereta harian. Berguna sebagai aksesori tambahan yang praktikal dan membantu perjalanan.'};
    }
    ...
    if(has(/ps5|ps4|xbox|nintendo|switch|console|controller|gamepad|gaming chair|gaming desk|game\b|games\b/)){
      return {category:'gaming', badge:'Gaming Gear', icon:'ðŸŽ®', meta:'Best for gaming setup', desc:'sesuai untuk setup gaming dan hiburan. Berguna untuk pengalaman bermain yang lebih selesa.'};
    }

    if(has(/watch|smartwatch|smart watch|jam tangan|casio|seiko|g-shock|gshock/)){
      return {category:'watches', badge:'Watch Pick', icon:'âŒš', meta:'Daily watch and style item', desc:'sesuai untuk kegunaan harian, gaya dan kemudahan semak masa atau fungsi pintar.'};
    }

    if(has(/handbag|tote bag|women bag|beg wanita|purse|sling bag wanita|shoulder bag/)){
      return {category:'womens-bags', badge:'Bag Pick', icon:'ðŸ‘œ', meta:'Useful bag for daily carry', desc:'sesuai untuk kegunaan harian, kerja atau travel. Mudah bawa barang penting dengan lebih kemas.'};
    }

    if(has(/wallet|dompet|men bag|beg lelaki|sling bag lelaki|crossbody bag|card holder/)){
      return {category:'mens-bags', badge:'Daily Carry', icon:'ðŸ‘', meta:'Best for wallet and daily carry', desc:'sesuai untuk simpan barang penting harian seperti kad, duit dan aksesori kecil.'};
    }

    if(has(/dress|blouse|skirt|women clothes|baju perempuan|kurung|kebaya|abaya|jubah wanita/)){
      return {category:'women-clothes', badge:'Women Fashion', icon:'ðŸ‘—', meta:'Popular fashion item', desc:'sesuai untuk gaya harian, kerja atau majlis. Pilihan pakaian wanita yang mudah dipadankan.'};
    }

    if(has(/shirt|tshirt|t-shirt|polo|men clothes|baju lelaki|seluar lelaki|pants|jeans|shorts|hoodie|jacket/)){
      return {category:'men-clothes', badge:'Men Fashion', icon:'ðŸ‘•', meta:'Daily men fashion item', desc:'sesuai untuk pakaian harian, santai atau kerja. Mudah dipadankan dengan gaya ringkas.'};
    }

    if(has(/tudung|hijab|shawl|telekung|kopiah|sejadah|muslim|jubah|abaya/)){
      return {category:'muslim', badge:'Muslim Fashion', icon:'ðŸ§•', meta:'Useful Muslim fashion item', desc:'sesuai untuk kegunaan harian, ibadah atau gaya muslimah/muslim yang kemas.'};
    }

    if(has(/women shoe|heels|high heel|flat shoe|kasut wanita|sandal wanita/)){
      return {category:'women-shoes', badge:'Women Shoes', icon:'ðŸ‘ ', meta:'Daily footwear pick', desc:'sesuai untuk kegunaan harian, kerja atau gaya santai. Pilihan kasut yang mudah dipadankan.'};
    }

    if(has(/shoe|shoes|sneaker|kasut|sandal|slipper|boots/)){
      return {category:'men-shoes', badge:'Shoes Pick', icon:'ðŸ‘Ÿ', meta:'Daily footwear pick', desc:'sesuai untuk kegunaan harian, berjalan atau gaya santai. Pilihan kasut yang praktikal.'};
    }
    ...
    if(has(/beauty|skincare|skin care|makeup|serum|sunscreen|moisturizer|cleanser|lipstick|perfume|health|supplement|masker|facial|shampoo|hair dryer|trimmer|shaver/)){
      return {category:'health', badge:'Self Care', icon:'âœ¨', meta:'Best for self care and daily routine', desc:'sesuai untuk penjagaan diri, kesihatan atau rutin harian. Semak kesesuaian sebelum membeli.'};
    }

    if(has(/baby|kids|kid|toy|toys|mainan|stroller|milk bottle|botol susu|diaper|lampin|school bag|beg sekolah/)){
      return {category:'baby', badge:'Baby & Kids', icon:'ðŸ§¸', meta:'Useful for baby and kids', desc:'sesuai untuk bayi, kanak-kanak atau kegunaan keluarga. Praktikal untuk keperluan harian.'};
    }
    ...
    if(has(/food|coklat|chocolate|snack|biscuit|cookies|kopi|coffee|tea|grocery|groceries|minuman|makanan|instant noodle|pet food|cat food|dog food|kibble/)){
      return {category:'groceries', badge: has(/chocolate|coklat/) ? 'Chocolate' : 'Groceries', icon:'ðŸ«', meta:'Best for snack, grocery or daily stock', desc:'sesuai untuk stok harian, kudapan atau keperluan rumah. Semak detail produk sebelum membeli.'};
    }
    ...
    if(has(/gym|dumbbell|fitness|yoga|cycling|bicycle|sport|sports|outdoor|camping|camp|tent|hiking|fishing|badminton|football/)){
      return {category:'sports', badge:'Sports & Outdoor', icon:'ðŸ•ï¸', meta:'Best for workout and outdoor activity', desc:'sesuai untuk aktiviti sukan, workout atau outdoor. Berguna untuk gaya hidup aktif.'};
    }

    if(has(/book|books|novel|komik|comic|stationery|alat tulis|hobby|puzzle|lego|model kit/)){
      return {category:'books', badge:'Books & Hobby', icon:'ðŸ“š', meta:'Best for reading and hobby', desc:'sesuai untuk bacaan, pembelajaran atau hobi masa lapang.'};
    }

    if(has(/travel|luggage|bagasi|suitcase|passport holder|neck pillow|travel bag|organizer travel/)){
      return {category:'travel', badge:'Travel Essential', icon:'ðŸ§³', meta:'Best for travel and packing', desc:'sesuai untuk travel, balik kampung atau susun barang perjalanan dengan lebih kemas.'};
    }

    if(has(/ticket|voucher|coupon|topup|reload|gift card/)){
      return {category:'tickets', badge:'Voucher', icon:'ðŸŽŸï¸', meta:'Ticket, voucher or digital item', desc:'sesuai untuk baucar, tiket atau item digital. Semak terma dan syarat sebelum membeli.'};
    }

    return {category:'others', badge:'Useful Item', icon:'ðŸ›’', meta:'Best for useful daily item', desc:'sesuai untuk kegunaan harian. Semak detail produk di Shopee sebelum membeli.'};
  }
  ...
  function autoFillAffiliateFromTitle(){
    ...
    setTitleFillStatus('âœ… Auto filled daripada tajuk produk. Sila semak sebelum Save.', false);
  }
  ...
  async function autoDetectAffiliateProduct(){
    ...
    try{
      ...
      if(btn){
        btn.disabled = false;
        btn.textContent = oldText || 'ðŸ” Auto Detect';
      }
    }
  }
  ...
  async function copyShopeeJsonExtractorScript(){
    ...
    try{
      ...
      setJsonImportStatus('âœ… Console script copied. Manual fallback: buka Shopee â†’ F12 Console â†’ paste â†’ Enter. JSON akan auto download.', false);
    }catch(e){
      setJsonImportStatus('Copy gagal. Browser block clipboard. Sila cuba lagi atau guna extension auto JSON.', true);
    }
  }
  ...
  async function openShopeeForJsonExtension(){
    ...
    try{
      localStorage.setItem('azobss_pending_shopee_json_link', url);
    }catch(e){}

    const win = window.open(url, '_blank', 'noopener,noreferrer');
    if(copied){
      setJsonImportStatus('âœ… Console script copied + Shopee dibuka. Di Shopee: tekan F12 â†’ Console â†’ Ctrl+V â†’ Enter. JSON akan auto download. Jika extension dipasang, JSON boleh auto download tanpa console.', false);
    }else{
      setJsonImportStatus('Shopee dibuka, tetapi browser block copy script. Tekan button Copy Console Script dahulu, kemudian F12 â†’ Console â†’ Ctrl+V â†’ Enter.', true);
    }
    if(!win){
  ...
  function applyShopeeJsonToForm(raw){
    ...
    setJsonImportStatus('âœ… JSON imported. Form auto filled daripada data Shopee. Sila semak sebelum Save.', false);
  }
  ...
  function bindAdmin(){
    ...
    // Shopee JSON Auto Import buttons
    ...
    qs('#affiliateImportJsonButton')?.addEventListener('click', () => {
      if(!adminDetected()) return;
      const input = qs('#affiliateShopeeJsonFile');
      if(input){
        input.value = '';
        input.click();
      }
    ...
    qs('#affiliateShopeeJsonFile')?.addEventListener('change', function(){
      ...
      if(file) importShopeeJsonFile(file);
    });
    ...
    // â”€â”€ Deploy to GitHub button â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
