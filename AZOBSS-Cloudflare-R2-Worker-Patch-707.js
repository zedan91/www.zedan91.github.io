// AZOBSS Cloudflare R2 Download Gateway - Patch 707
...
const ALLOWED_PREFIXES = ["software/", "cad/"];
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname === "/health") {
        return json({
          ok: true,
          patch: "AZOBSS_R2_WORKER_DIRECT_GATE_707_20260731",
      ...
      return text(error?.message || "Gateway error", Number(error?.status || 500));
    }
  }
};
...
function json(data, status = 200) {
  return new Response(JSON.stringify(data, null, 2), { status, headers: noStoreHeaders({ "content-type": "application/json; charset=utf-8" }) });
}
