...
function setupAnimeSourceFallback(series,ep,check){
  ...
  if(openBtn){
    openBtn.textContent='Watch Source';
    openBtn.href=ep.url;
  ...
  if(blocked){
    ...
    blocked.innerHTML=`<div class="anime-live-fallback-poster" aria-hidden="true"></div>
      ...
      <div class="anime-live-fallback-content">
        <button type="button" class="anime-live-fallback-play" id="animeSourceFallbackBtn" aria-label="Watch source">▶</button>
        ...
        <span class="anime-live-fallback-note">This provider blocks in-page playback. Watch Source opens in this same browser tab; use Back to return to AZOBSSTV.</span>
      </div>`;
    ...
    $('#animeSourceFallbackBtn')?.addEventListener('click',()=>openAnimeSourceSameTab(ep));
  }

  if(status)status.textContent='Source blocks in-page playback • same-tab fallback ready';
}
...
async function loadPlaylist(url,name='AZOBSSTV Free'){
  ...
  if(isDefaultFree&& !parsed.length){
    try{
      const raw=await localTextGet('./data/free.m3u?v=1009',5000);
      parsed=parseM3U(raw);
      if(parsed.length){
...
window.addEventListener('DOMContentLoaded',()=>{bindEnglishAZOBSSTVNavigation();bind();startHeroSideSync();boot();if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=1009').catch(()=>{});if(document.visibilityState==='visible')startForegroundLoops()});
})();
