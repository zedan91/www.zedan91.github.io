...
async function loadPlaylist(url,name='AZOBSSTV Free'){
  ...
  if(isDefaultFree&& !parsed.length){
    try{
      const raw=await textDirect('./data/free.m3u?v=1005',15000);
      parsed=parseM3U(raw);
      if(parsed.length){
...
window.addEventListener('DOMContentLoaded',()=>{bindEnglishAZOBSSTVNavigation();bind();startHeroSideSync();boot();if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=1005').catch(()=>{});if(document.visibilityState==='visible')startForegroundLoops()});
})();
