...
const LOCAL_LIVE_ARTWORK={
  ...
  'manis-fm':'./assets/channel-icons/radio-manis-fm-card.svg',
  'suria-fm':'./assets/channel-icons/radio-suria-fm-card.svg',
  'fly-fm':'./assets/channel-icons/radio-fly-fm-card.svg',
  'rakita-fm':'./assets/channel-icons/radio-rakita-fm-card.svg',
  'hot-fm':'./assets/channel-icons/radio-hot-fm-card.svg',
  'ikim-fm':'./assets/channel-icons/radio-ikim-fm-card.svg',
  '988-fm':'./assets/channel-icons/radio-988-fm-card.svg',
  'molek-fm':'./assets/channel-icons/radio-molek-fm-card.svg',
  'eight-fm':'./assets/channel-icons/radio-eight-fm-card.svg',
  'kool-fm':'./assets/channel-icons/radio-kool-fm-card.svg',
  'nasional-fm':'./assets/channel-icons/radio-nasional-fm-card.svg',
  'traxx-fm':'./assets/channel-icons/radio-traxx-fm-card.svg',
  'minnal-fm':'./assets/channel-icons/radio-minnal-fm-card.svg',
  'ai-fm':'./assets/channel-icons/radio-ai-fm-card.svg',
  'radio-klasik':'./assets/channel-icons/radio-radio-klasik-card.svg',
  'sabah-fm':'./assets/channel-icons/radio-sabah-fm-card.svg',
  'sabahv-fm':'./assets/channel-icons/radio-sabahv-fm-card.svg',
  'sarawak-fm':'./assets/channel-icons/radio-sarawak-fm-card.svg',
  'bernama-radio':'./assets/channel-icons/radio-bernama-radio-card.svg',
  'wai-fm':'./assets/channel-icons/radio-wai-fm-card.svg',
  'asyik-fm':'./assets/channel-icons/radio-asyik-fm-card.svg',
  'best-fm':'./assets/channel-icons/radio-best-fm-card.svg'
};

// v1020: Movies are rendered as a native AZOBSSTV catalogue from a local
// metadata snapshot. 1Tube is now the Movies metadata/watch provider;
...
function applyLocalLiveArtwork(c){
  ...
  // v1085: Radio cards always paint bundled/local station artwork first.
  // Provider artwork is only a background upgrade and must never compete with
  // the first visible Radio/Anime UI paint.
  if(String(c.kind||'').toLowerCase()==='radio'){
    const upgrade=String(c.remoteLogo||'').trim();
    const primary=local||remote||'';
    const backgroundUpgrade=local?(upgrade||((remote&&remote!==local)?remote:'')):upgrade;
    if(primary)return {...c,logo:primary,remoteLogo:backgroundUpgrade,fallbackLogo:''};
    return {...c,logo:'',remoteLogo:backgroundUpgrade,fallbackLogo:''};
  }
  if(!local)return c;
...
// v1030 — Radio-Online.my supplies catalogue/artwork only. Actual listening is
...
const RADIO_STREAM_CACHE_PREFIX='azobsstv_radio_stream_v1030:';
...
// v1030 — true audio-reactive equalizer. Bars are driven by frequency data
// captured from the *actual* native radio audio element. No random/keyframe
// animation is used. captureStream() is deliberately preferred because it
// does not reroute or mute cross-origin audio playback if analysis is denied.
function radioEqualizerBars(){return $$('#radioPlayerPanel .radio-equalizer i')}
function resetRadioEqualizer(mode='idle'){
  if(state.radioMeterRaf){cancelAnimationFrame(state.radioMeterRaf);state.radioMeterRaf=0}
  if(state.radioMeterSource){try{state.radioMeterSource.disconnect()}catch{}state.radioMeterSource=null}
  state.radioAnalyser=null;state.radioMeterData=null;state.radioMeterAttached=false;
  // Do not stop captured tracks here; disconnecting the analyser is enough and
  // avoids any browser-specific chance of affecting the underlying radio audio.
  state.radioMeterStream=null;
  const eq=$('#radioPlayerPanel .radio-equalizer');
  if(eq){eq.classList.toggle('meter-unavailable',mode==='unavailable');eq.dataset.meterState=mode;eq.title=mode==='unavailable'?'Live spectrum is unavailable for this station/browser. The bars will not fake movement.':mode==='live'?'Live spectrum from the current radio audio.':''}
  for(const bar of radioEqualizerBars()){bar.style.height='10px';bar.style.opacity=mode==='live'?'1':'0.42';bar.style.filter='';bar.__azLevel=0}
}
function primeRadioAudioMeter(){
  try{
    const Ctx=window.AudioContext||window.webkitAudioContext;if(!Ctx)return false;
    if(!state.radioAudioContext)state.radioAudioContext=new Ctx({latencyHint:'interactive'});
    state.radioMeterPrimed=true;
    if(state.radioAudioContext.state==='suspended')state.radioAudioContext.resume().catch(()=>{});
    return true;
  }catch(e){console.debug('AZOBSSTV radio meter context unavailable:',e?.message||e);return false}
}
function drawRadioSpectrum(){
  const analyser=state.radioAnalyser,data=state.radioMeterData,bars=radioEqualizerBars();
  if(!analyser||!data||!bars.length||!state.radioMeterAttached)return;
  analyser.getByteFrequencyData(data);
  const ctx=state.radioAudioContext;
  const nyquist=Math.max(1,(ctx?.sampleRate||48000)/2);
  const minHz=55,maxHz=Math.min(14000,nyquist*.92),count=bars.length;
  for(let i=0;i<count;i++){
    const f0=minHz*Math.pow(maxHz/minHz,i/count),f1=minHz*Math.pow(maxHz/minHz,(i+1)/count);
    let b0=Math.max(0,Math.min(data.length-1,Math.floor(f0/nyquist*data.length)));
    let b1=Math.max(b0+1,Math.min(data.length,Math.ceil(f1/nyquist*data.length)));
    let sum=0,peak=0,n=0;
    for(let b=b0;b<b1;b++){const v=data[b];sum+=v;if(v>peak)peak=v;n++}
    const avg=n?sum/n:0;
    // Blend RMS-like average with the local peak so speech and bass transients
    // are visible without making all bars jump to full height.
    const raw=Math.max(0,Math.min(1,((avg*.72+peak*.28)-5)/190));
    const prev=Number(bars[i].__azLevel||0),attack=.64,decay=.18;
    const level=raw>prev?prev+(raw-prev)*attack:prev+(raw-prev)*decay;
    bars[i].__azLevel=level;
    const maxPx=window.innerWidth<=760?44:60,minPx=7;
    bars[i].style.height=(minPx+Math.pow(level,.72)*(maxPx-minPx)).toFixed(1)+'px';
    bars[i].style.opacity=String(.34+level*.66);
    bars[i].style.filter=level>.62?`brightness(${(1+level*.28).toFixed(2)})`:'';
  }
  state.radioMeterRaf=requestAnimationFrame(drawRadioSpectrum);
}
function attachRadioAudioMeter(audio){
  resetRadioEqualizer('idle');
  if(!audio||audio.paused)return false;
  if(!primeRadioAudioMeter()){resetRadioEqualizer('unavailable');return false}
  const capture=audio.captureStream||audio.mozCaptureStream;
  if(typeof capture!=='function'){resetRadioEqualizer('unavailable');return false}
  try{
    const stream=capture.call(audio),tracks=stream?.getAudioTracks?.()||[];
    if(!tracks.length){resetRadioEqualizer('unavailable');return false}
    const ctx=state.radioAudioContext;if(ctx.state==='suspended')ctx.resume().catch(()=>{});const source=ctx.createMediaStreamSource(stream),analyser=ctx.createAnalyser();
    analyser.fftSize=512;analyser.smoothingTimeConstant=.66;analyser.minDecibels=-92;analyser.maxDecibels=-12;
    source.connect(analyser);
    state.radioMeterStream=stream;state.radioMeterSource=source;state.radioAnalyser=analyser;state.radioMeterData=new Uint8Array(analyser.frequencyBinCount);state.radioMeterAttached=true;
    const eq=$('#radioPlayerPanel .radio-equalizer');if(eq){eq.classList.remove('meter-unavailable');eq.dataset.meterState='live';eq.title='Live spectrum from the current radio audio.'}
    for(const track of tracks){track.addEventListener?.('mute',()=>{if(state.radioMeterStream===stream)resetRadioEqualizer('unavailable')},{once:true})}
    drawRadioSpectrum();return true;
  }catch(e){console.debug('AZOBSSTV live radio spectrum unavailable:',e?.message||e);resetRadioEqualizer('unavailable');return false}
}
...
function stopRadioAudioPlayback(clear=true,message=''){
  state.radioResolveId++;stopRadioHls();resetRadioEqualizer('idle');const a=$('#azobsstvRadioAudio');if(a){try{a.pause()}catch{}if(clear){try{a.removeAttribute('src');a.load()}catch{}}}
...
async function playRadioCandidate(c,url,requestId){
  ...
  if(!a.paused)attachRadioAudioMeter(a);
  return !a.paused;
}
...
// v1037 — Artwork must never hold the UI hostage while a remote resolver is
// sleeping. Cards get an immediate local/generated image and real provider
// artwork is upgraded in the background when it becomes available.
function artworkFallbackData(name='',subtitle='',kind='media'){
  const label=String(name||'AZ').trim();
  const words=label.split(/\s+/).filter(Boolean);
  const initials=(words.slice(0,2).map(w=>w[0]).join('')||label.slice(0,2)||'AZ').toUpperCase().slice(0,3);
  const hue=[...label].reduce((n,ch)=>(n*33+ch.charCodeAt(0))%360,215);
  const h2=(hue+58)%360;
  const safe=(v)=>String(v||'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&apos;'}[c]));
  const title=safe(initials),sub=safe(String(subtitle||kind||'').slice(0,26));
  const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="360" height="480" viewBox="0 0 360 480"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="hsl(${hue} 72% 42%)"/><stop offset="1" stop-color="hsl(${h2} 70% 24%)"/></linearGradient><radialGradient id="r" cx="70%" cy="20%" r="75%"><stop stop-color="rgba(255,255,255,.22)"/><stop offset="1" stop-color="rgba(255,255,255,0)"/></radialGradient></defs><rect width="360" height="480" rx="34" fill="url(#g)"/><rect width="360" height="480" rx="34" fill="url(#r)"/><circle cx="180" cy="205" r="92" fill="rgba(5,10,24,.30)" stroke="rgba(255,255,255,.20)" stroke-width="3"/><text x="180" y="230" text-anchor="middle" fill="white" font-family="Arial,sans-serif" font-size="74" font-weight="900">${title}</text><text x="180" y="405" text-anchor="middle" fill="rgba(255,255,255,.9)" font-family="Arial,sans-serif" font-size="22" font-weight="700">${sub}</text></svg>`;
  return 'data:image/svg+xml;charset=UTF-8,'+encodeURIComponent(svg);
}
function usableArtworkUrl(v){const s=String(v||'').trim();return s&&!/\/imgs\/poster\/no_poster\.jpg(?:\?|$)/i.test(s)?s:''}
function startRemoteArtworkUpgrade(img){
  if(!img||img.tagName!=='IMG')return;
  const remote=String(img.dataset.remoteLogo||'').trim();
  if(!remote||remote===img.getAttribute('src')||img.dataset.azRemoteTried)return;
  img.dataset.azRemoteTried='1';
  const run=()=>{
    const probe=new Image();probe.referrerPolicy='no-referrer';
    let done=false;const timer=setTimeout(()=>{done=true;probe.src=''},6500);
    probe.onload=()=>{if(done)return;done=true;clearTimeout(timer);if((probe.naturalWidth||0)>=24&&(probe.naturalHeight||0)>=24)img.src=remote};
    probe.onerror=()=>{if(done)return;done=true;clearTimeout(timer)};
    probe.src=remote;
  };
  if('requestIdleCallback'in window)requestIdleCallback(run,{timeout:1400});else setTimeout(run,420);
}
function hydrateRemoteArtwork(selector){
  const imgs=$$(selector).filter(x=>x.tagName==='IMG'&&String(x.dataset.remoteLogo||'').trim()&&!x.dataset.azRemoteTried);
  if(!imgs.length)return;
  if('IntersectionObserver'in window){
    if(!state.artworkObserver)state.artworkObserver=new IntersectionObserver(entries=>{
      entries.forEach(entry=>{if(!entry.isIntersecting)return;state.artworkObserver.unobserve(entry.target);startRemoteArtworkUpgrade(entry.target)});
    },{rootMargin:'320px 0px'});
    imgs.forEach(img=>state.artworkObserver.observe(img));
  }else imgs.slice(0,24).forEach(startRemoteArtworkUpgrade);
}
...
function normalizeRadioOnlineRow(x,index=0){
  ...
  const rawLogo=usableArtworkUrl(x?.logo||x?.image||'');
  ...
  const directProviderLogo=rawLogo&&!/\/logo\.webp(?:\?|$)/i.test(rawLogo)?rawLogo:'';
  // v1085: bundled SVG/PNG artwork is the first-paint source. The provider
  // logo/resolver is a later viewport-only upgrade.
  const logo=fallbackLogo||directProviderLogo||'';
  const remoteLogo=fallbackLogo?(directProviderLogo||resolver):(directProviderLogo?'':resolver);
  return{
    name,logo,remoteLogo,fallbackLogo:'',
...
async function loadRadioOnlineFallback(){
  try{
    const data=await localJget('./data/radio-online-my-catalog.json?v=1085',8000);
...
async function loadMana2PublicCatalog(){
  ...
  // v1030: Mana-Mana is now Live TV only inside AZOBSSTV. Radio is replaced by
...
// movies-1tube-catalog.json at build time so Movies cannot briefly disappear while
// Live TV/Radio/background sync is still resolving.
const EMBEDDED_MOVIE_FALLBACK=[{"id":"1tube-movie-969681","tmdbId":"969681","name":"Spider-Man: Brand New Day","year":2026,"rating":"7.9","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2F63JYqhefGrbVg0YqZ3xWqn0pwT2.jpg","sourcePage":"https://www.1tube.org/watch/969681","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"21:15","views":"138K"},{"id":"1tube-movie-1084244","tmdbId":"1084244","name":"Toy Story 5","year":2026,"rating":"8.0","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2FowUcU7WcM42uPXIVJHglDQceY5R.jpg","sourcePage":"https://www.1tube.org/watch/1084244","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"1:42:00","studio":"Pixar","views":"14.7K"},{"id":"1tube-movie-1368337","tmdbId":"1368337","name":"The Odyssey","year":2026,"rating":"8.0","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2FqaWgqQdJwohmc4XK4iXAiZiPOKH.jpg","sourcePage":"https://www.1tube.org/watch/1368337","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"2:53:00","studio":"Universal Pictures","views":"55.3K"},{"id":"1tube-movie-1315772","tmdbId":"1315772","name":"Minions & Monsters","year":2026,"rating":"7.5","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2FsfGdosEmaYJQha8esMcP8YuCpaj.jpg","sourcePage":"https://www.1tube.org/watch/1315772","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"1:30:00","studio":"Universal Pictures","views":"21.4K"},{"id":"1tube-movie-1101383","tmdbId":"1101383","name":"The End of Oak Street","year":2026,"rating":"6.5","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2F3bq3Jy5cnE2FjngeRVvkJawquVf.jpg","sourcePage":"https://www.1tube.org/watch/1101383","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"1:39:00","studio":"Warner Bros. Pictures","views":"34.3K"},{"id":"1tube-movie-102382","tmdbId":"102382","name":"The Amazing Spider-Man 2","year":2014,"rating":"6.6","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2FdPj9RAHYhcmUQfLR7HKgthOzxgZ.jpg","sourcePage":"https://www.1tube.org/watch/102382","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"2:21:00","studio":"Columbia Pictures","views":"4K"},{"id":"1tube-movie-980431","tmdbId":"980431","name":"Avatar Aang: The Last Airbender","year":2026,"rating":"9.2","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2F74iiUwzqe2BnRZl7QD4LVQUQKHb.jpg","sourcePage":"https://www.1tube.org/watch/980431","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"1:39:00","studio":"Nickelodeon Movies","views":"7K"},{"id":"1tube-movie-1391021","tmdbId":"1391021","name":"Once Upon a Time in the Middle East","year":2026,"rating":"9.2","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2Fzr7om0GOLOcLo7SXMmNEG5rz6de.jpg","sourcePage":"https://www.1tube.org/watch/1391021","categories":["Movies","1Tube"],"sourceProvider":"1tube"},{"id":"1tube-movie-1632181","tmdbId":"1632181","name":"Accidental Partners","year":2026,"rating":"8.9","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2Fh19ZHvxGnpqSzm2JyHEwMhl7zRs.jpg","sourcePage":"https://www.1tube.org/watch/1632181","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"1:50:00","studio":"Videocine","views":"0"},{"id":"1tube-movie-1007757","tmdbId":"1007757","name":"Swapped","year":2026,"rating":"8.9","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2FidSpi0uC5d1eulFxMrKI9iY2Qg8.jpg","sourcePage":"https://www.1tube.org/watch/1007757","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"1:42:00","studio":"Skydance Animation","views":"1.1K"},{"id":"1tube-movie-936075","tmdbId":"936075","name":"Michael","year":2026,"rating":"8.7","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2FjoPZFstfdjpsuzlrnGeCbY0FuPS.jpg","sourcePage":"https://www.1tube.org/watch/936075","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"2:08:00","studio":"Lionsgate","views":"3.4K"},{"id":"1tube-movie-687163","tmdbId":"687163","name":"Project Hail Mary","year":2026,"rating":"8.7","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2FwLWvHcQz7N13DvSOTV7bHgwGXpT.jpg","sourcePage":"https://www.1tube.org/watch/687163","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"2:37:00","studio":"Lord Miller","views":"6.2K"},{"id":"1tube-movie-1408162","tmdbId":"1408162","name":"Vishwanath & Sons","year":2026,"rating":"8.4","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2Fsd0RKOpnqESIWxU3sZwZhBsgAHl.jpg","sourcePage":"https://www.1tube.org/watch/1408162","categories":["Movies","1Tube"],"sourceProvider":"1tube"},{"id":"1tube-movie-278","tmdbId":"278","name":"The Shawshank Redemption","year":1994,"rating":"8.7","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2FxXXFdqV965crlxCO3dj3PhtcCAf.jpg","sourcePage":"https://www.1tube.org/watch/278","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"2:22:00","studio":"Castle Rock Entertainment","views":"1.4K"},{"id":"1tube-movie-238","tmdbId":"238","name":"The Godfather","year":1972,"rating":"8.7","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2FAbgEQO2mneCSOc8CSnOMa8pBS8I.jpg","sourcePage":"https://www.1tube.org/watch/238","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"2:55:00","studio":"Paramount Pictures","views":"1.4K"},{"id":"1tube-movie-1323244","tmdbId":"1323244","name":"Rage of Stars","year":2026,"rating":"4.8","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2Fz7lZgL5tzefTfhyRtouThFhsuUS.jpg","sourcePage":"https://www.1tube.org/watch/1323244","categories":["Movies","1Tube"],"sourceProvider":"1tube"},{"id":"1tube-movie-1375646","tmdbId":"1375646","name":"Colony","year":2026,"rating":"8.1","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2FmXhgb6WzczIJPOJSlY2hhUZuwc7.jpg","sourcePage":"https://www.1tube.org/watch/1375646","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"2:03:00","studio":"Wow Point","views":"9.6K"},{"id":"1tube-movie-1284041","tmdbId":"1284041","name":"The Last House","year":2026,"rating":"6.9","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2F9wCrNgVcaOhS5jPK9B5reFIWvxh.jpg","sourcePage":"https://www.1tube.org/watch/1284041","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"1:50:00","studio":"Chernin Entertainment","views":"11.4K"},{"id":"1tube-movie-1339713","tmdbId":"1339713","name":"Obsession","year":2026,"rating":"8.2","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2Fe1qJyp8AE5ejXAaQjxWPTsWeTb.jpg","sourcePage":"https://www.1tube.org/watch/1339713","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"1:49:00","studio":"Tea Shop Productions","views":"28.1K"},{"id":"1tube-movie-1284465","tmdbId":"1284465","name":"The Death of Robin Hood","year":2026,"rating":"6.3","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2FugOolHi7NNSoSclURMEcuMAIAl7.jpg","sourcePage":"https://www.1tube.org/watch/1284465","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"2:03:00","studio":"Lyrical Media","views":"3.7K"},{"id":"1tube-movie-1108427","tmdbId":"1108427","name":"Moana","year":2026,"rating":"6.1","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2F7gidpHWeWlnjJOgQQsQIxwJs2Pp.jpg","sourcePage":"https://www.1tube.org/watch/1108427","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"1:55:00","studio":"Walt Disney Pictures","views":"4.5K"},{"id":"1tube-movie-1212763","tmdbId":"1212763","name":"Evil Dead Burn","year":2026,"rating":"7.9","logo":"https://wsrv.nl/?url=https%3A%2F%2Fimage.tmdb.org%2Ft%2Fp%2Fw780%2FviHFZ7GlzMcmPbADvP4Vd6Ema0g.jpg","sourcePage":"https://www.1tube.org/watch/1212763","categories":["Movies","1Tube"],"sourceProvider":"1tube","duration":"1:50:00","studio":"New Line Cinema","views":"8.6K"}];
...
function normalizeMovieCategories(values){
  const src=Array.isArray(values)?values:[];
  const out=[];
  for(const raw of src){
    let value='';
    if(raw&&typeof raw==='object')value=String(raw.name??raw.title??raw.id??'').trim();
    else value=String(raw??'').trim();
    if(!value)continue;
    const lower=value.toLowerCase();
    if(['movies','movie','1tube','1 tube','1-tube'].includes(lower))continue;
    if(/^\d+$/.test(value))value=TMDB_MOVIE_GENRES[value]||'';
    if(value&&!out.includes(value))out.push(value);
  }
  return out;
}
function normalizeMovieRows(data){
  const rows=Array.isArray(data?.items)?data.items:[];
  const liveSource=data?.source==='1tube-live';
  return rows.map((x,index)=>{
    const tmdbId=String(x?.tmdbId||x?.id||'').replace(/^1tube-movie-/,'').trim();
    const source=String(x?.sourcePage||x?.url||'').trim();
    const url=source||(/^[0-9]+$/.test(tmdbId)?`https://www.1tube.org/watch/${encodeURIComponent(tmdbId)}`:'https://www.1tube.org/movies');
    const name=String(x?.name||x?.title||`Movie ${index+1}`).trim();
    const artwork=String(x?.logo||x?.poster||x?.image||'').trim();
    const year=x?.year??null;
    const rating=String(x?.rating||'').trim();
    if(!url||!name||!/^[0-9]+$/.test(tmdbId))return null;
    if(liveSource&&(!artwork||(!year&&!rating)))return null;
    const categories=normalizeMovieCategories(Array.isArray(x?.categories)?x.categories:(Array.isArray(x?.genres)?x.genres:(Array.isArray(x?.genre_ids)?x.genre_ids:[])));
    return{
      name,logo:artwork,
      id:String(x?.id||`1tube-movie-${tmdbId||index+1}`).trim(),
      group:'Movies',kind:'movies',categories,
      year,rating,
      duration:String(x?.duration||'').trim(),studio:String(x?.studio||'').trim(),views:String(x?.views||'').trim(),
      tmdbId,sourceProvider:'1tube',
...
function embeddedMovieCatalog(){
  return normalizeMovieRows({items:EMBEDDED_MOVIE_FALLBACK});
}
function rememberMovieCatalog(rows){
  const clean=Array.isArray(rows)?rows.filter(c=>c&&mediaType(c)==='movies'):[];
  if(clean.length)state.movieCatalogSnapshot=clean.slice();
  return clean;
}
function ensureMovieCatalogPresent(){
  const existing=state.channels.filter(c=>mediaType(c)==='movies');
  if(existing.length){
    if(!state.movieCatalogSnapshot.length)state.movieCatalogSnapshot=existing.slice();
    return false;
  }
  const fallback=state.movieCatalogSnapshot.length?state.movieCatalogSnapshot:embeddedMovieCatalog();
  if(!fallback.length)return false;
  state.movieCatalogSnapshot=fallback.slice();
  state.channels=[...state.channels,...fallback];
  return true;
}
async function loadLocalMovieCatalog(){
  const embedded=embeddedMovieCatalog();
  try{
    const data=await localJget('./data/movies-1tube-catalog.json?v=1085',3500);
    const rows=normalizeMovieRows(data);
    return rememberMovieCatalog(rows.length?rows:embedded);
  }catch(e){
    console.warn('AZOBSSTV Movies local catalogue fallback:',e?.message||e);
    return rememberMovieCatalog(embedded);
  }
}
async function loadRemoteMovieCatalog(){
  try{
    const remote=await jget(API_BASE+'/1tube/movies?pages=4',25000);
    if(Array.isArray(remote?.movies)&&remote.movies.length){const rows=normalizeMovieRows({items:remote.movies,source:'1tube-live'});if(rows.length)return rememberMovieCatalog(rows)}
  }catch(e){console.warn('AZOBSSTV 1Tube live catalogue fallback:',e?.message||e)}
  return[];
}
async function loadMovieCatalog(){
  const remote=await loadRemoteMovieCatalog();
  return remote.length?remote:await loadLocalMovieCatalog();
}
async function loadAnimeCatalog(){
  try{
    const data=await localJget('./data/anime-catalog.json?v=1085',8000);
    const rows=Array.isArray(data?.items)?data.items:[];
    return rows.map((x,index)=>{
      const url=String(x?.sourcePage||x?.url||'').trim();
      const name=String(x?.name||`Anime ${index+1}`).trim();
      const slug=String(x?.slug||'').trim();
      if(!url||!name)return null;
      const categories=Array.isArray(x?.categories)?x.categories.map(v=>String(v||'').trim()).filter(Boolean):[];
      const bundled=usableArtworkUrl(x?.logo||'');
      const posterResolver=slug?API_BASE+'/anime123/poster?slug='+encodeURIComponent(slug):'';
      // v1085: do not build 2,500+ SVG data URIs while parsing the catalogue.
      // A lightweight placeholder is generated only for cards that are actually
      // rendered, while the real poster is upgraded in the background.
      return{name,logo:bundled||'',remoteLogo:posterResolver,fallbackLogo:'',id:String(x?.id||x?.slug||`anime-${index+1}`).trim(),group:'Anime',kind:'series',categories,year:x?.year??null,rating:'',episodeCount:Number(x?.episodeCount||0)||0,episodeBase:String(x?.episodeBase||'').trim(),episodes:[],sourceProvider:String(x?.sourceProvider||'123animehub'),sourcePage:url,webOnly:true,mode:'web',officialUrl:url,altUrl:'',url,headers:{userAgent:'',referer:'',origin:'',authorization:''},drm:{licenseType:'',licenseKey:''},slug,status:String(x?.status||''),animeType:String(x?.type||''),tags:Array.isArray(x?.tags)?x.tags:[]};
    }).filter(Boolean);
  }catch(e){console.warn('AZOBSSTV anime catalogue fallback:',e?.message||e);return[]}
}
...
function imageMarkup(c,cls,alt='',priority=false){
  const badge=channelBadgeText(c?.name);
  let src=String(c?.logo||'').trim();
  let fallbackLogo=String(c?.fallbackLogo||'').trim();
  const remoteLogo=String(c?.remoteLogo||'').trim();
  if(!src&&remoteLogo){
    if(!c._azRenderFallback)c._azRenderFallback=artworkFallbackData(c?.name,mediaType(c)==='series'?(c?.year?String(c.year):'ANIME'):(mediaType(c)==='radio'?'LIVE RADIO':''),mediaType(c));
    src=c._azRenderFallback;
  }
  if(!src)return `<div class="${cls} fallback">${esc(badge)}</div>`;
  const loading=priority?'eager':'lazy';
  const fetchPriority=priority?'high':'auto';
  return `<img class="${cls}" loading="${loading}" fetchpriority="${fetchPriority}" decoding="async" referrerpolicy="no-referrer" src="${esc(src)}" data-remote-logo="${esc(remoteLogo)}" data-fallback-logo="${esc(fallbackLogo)}" data-fallback="${esc(badge)}" alt="${esc(alt)}">`;
}
...
function initialCatalogLimit(tab){if(tab==='series')return 48;if(tab==='radio')return 56;if(tab==='movies')return 60;return 240}
function catalogPageStep(tab){if(tab==='series')return 48;if(tab==='radio')return 56;if(tab==='movies')return 60;return 120}
function ensureCatalogPager(){
  let pager=$('#catalogPager');if(pager)return pager;
  const grid=$('#channelGrid');if(!grid)return null;
  pager=document.createElement('div');pager.id='catalogPager';pager.className='catalog-pager';pager.hidden=true;
  pager.innerHTML='<button type="button" class="ghost-btn catalog-more-btn">Load more</button><span class="catalog-pager-text"></span>';
  grid.insertAdjacentElement('afterend',pager);
  pager.querySelector('button')?.addEventListener('click',()=>growCatalogPage());
  if('IntersectionObserver'in window){
    state.catalogPagerObserver=new IntersectionObserver(entries=>{if(entries.some(e=>e.isIntersecting))growCatalogPage()},{rootMargin:'700px 0px'});
    state.catalogPagerObserver.observe(pager);
  }
  return pager;
}
function updateCatalogPager(total,visible){
  const pager=ensureCatalogPager();if(!pager)return;
  const more=visible<total;pager.hidden=!more;
  const label=pager.querySelector('.catalog-pager-text');if(label)label.textContent=more?`Showing ${visible.toLocaleString()} of ${total.toLocaleString()}`:'';
}
function growCatalogPage(){
  const total=state.filtered.length;if(!total||state.catalogVisibleLimit>=total)return;
  state.catalogVisibleLimit=Math.min(total,state.catalogVisibleLimit+catalogPageStep(state.tab));
  render();
}
function renderChannelRail(list=railListForCurrentView()){
  ...
  const railLimit=(state.tab==='series'||state.tab==='radio')?48:100;
  let shown=items.slice(0,railLimit);
  if(state.current&&!shown.includes(state.current)&&items.includes(state.current))shown=[state.current,...shown.slice(0,Math.max(0,railLimit-1))];
  rail.innerHTML=shown.map((c,i)=>`<div class="channel-rail-card ${state.current&&state.current.url===c.url?'active':''}" role="listitem" data-rail-index="${i}" title="${esc(c.name)}">${imageMarkup(c,'channel-rail-logo','',i<10)}<div class="channel-rail-text"><div class="channel-rail-name">${esc(c.name)}</div></div><button class="channel-rail-fav ${state.favorites.has(channelKey(c))?'active':''}" type="button" data-rail-fav="${i}" aria-label="Favorite">♥</button></div>`).join('');
  bindImageFallbacks('#channelRailList img.channel-rail-logo');
  hydrateRemoteArtwork('#channelRailList img.channel-rail-logo');
  [...rail.querySelectorAll('.channel-rail-card')].forEach(el=>el.addEventListener('click',e=>{const i=Number(el.dataset.railIndex),c=shown[i];if(!c)return;if(e.target.closest('[data-rail-fav]')){toggleFav(c);e.stopPropagation();return}play(c)}));updateFavoriteAvailability();
}
...
function render(){
  ...
  state.filtered=list;
  const renderKey=[state.tab,q,grp,state.tab==='favorites'?'fav':state.tab==='recent'?'recent':'all'].join('|');
  if(state.catalogRenderKey!==renderKey){state.catalogRenderKey=renderKey;state.catalogVisibleLimit=initialCatalogLimit(state.tab)}
  if(!state.catalogVisibleLimit)state.catalogVisibleLimit=initialCatalogLimit(state.tab);
  const visibleList=list.slice(0,state.catalogVisibleLimit);
  ...
  grid.innerHTML=visibleList.map((c,i)=>{
    ...
    const img=imageMarkup(c,'channel-logo',c.name,i<12);
  ...
  hydrateRemoteArtwork('#channelGrid img.channel-logo');
  $$('.channel-card').forEach(el=>el.addEventListener('click',e=>{const i=Number(el.dataset.index);if(e.target.closest('[data-fav]')){toggleFav(visibleList[i]);e.stopPropagation();return}play(visibleList[i])}));
  updateCatalogPager(list.length,visibleList.length);
...
function movieAutoplayUrl(movie){
  const raw=String(movie?.sourcePage||movie?.url||'').trim();
  if(!raw)return '#';
  // 1Tube watch routes use the TMDB id directly; no 7Movies autoplay query is needed.
  if(String(movie?.sourceProvider||'').toLowerCase()==='1tube')return raw;
  try{const u=new URL(raw,location.href);u.searchParams.set('autoplay','1');return u.href}catch{return raw+(raw.includes('?')?'&':'?')+'autoplay=1'}
}
function renderMoviePlayer(movie){
  const panel=ensureMoviePlayerPanel();
  const wrap=$('#videoWrap');
  if(!panel||!wrap)return;
  const watchUrl=movieAutoplayUrl(movie);
  ...
  panel.innerHTML=`<div class="movie-player-backdrop" ${art?`style="background-image:linear-gradient(90deg,rgba(5,9,17,.94) 0%,rgba(5,9,17,.72) 46%,rgba(5,9,17,.16) 100%),url('${esc(art)}')"`:''}></div>
    <div class="movie-player-content">
      <span class="movie-player-kicker">MOVIE • 1TUBE</span>
      ...
      <p>Press Watch Movie to open the 1Tube watch page.</p>
      <div class="movie-player-actions">
        <a class="movie-player-open movie-watch-btn" href="${esc(watchUrl)}" target="_blank" rel="noopener noreferrer">▶ Watch Movie</a>
        <button class="movie-player-fav ${state.favorites.has(channelKey(movie))?'active':''}" type="button">♥ Favorite</button>
      </div>
    </div>
    <a class="movie-player-center-play" href="${esc(watchUrl)}" target="_blank" rel="noopener noreferrer" aria-label="Watch ${esc(movie.name||'movie')}">▶</a>`;
...
function showMovieDetail(movie,scroll=true){
  ...
  $('#nowMeta').textContent=cardSubline(movie)+' • 1Tube';
  ...
  if(box)box.innerHTML='<div class="today-schedule-empty">Movie selected. Press “▶ Watch Movie” to open the 1Tube watch page.</div>';
  if(scroll){
...
function hideRadioNativePlayer(){
...
function showRadioPlayer(c){
  // Create/resume the AudioContext while this function is still reached from
  // the user's station click. The actual analyser attaches only after audio plays.
  primeRadioAudioMeter();
  ...
  // v1030: do not load the Radio-Online.my page in a hidden iframe. That page
  ...
  if(restart)restart.onclick=()=>{primeRadioAudioMeter();restartRadioEngine(c)};
...
async function loadPlaylist(url,name='AZOBSSTV Free'){
  ...
  if(isDefaultFree&& !parsed.length){
    try{
      const raw=await localTextGet('./data/free.m3u?v=1085',5000);
      parsed=parseM3U(raw).map(applyLocalLiveArtwork).map(c=>({...c,kind:'live'}));
      if(parsed.length){
  ...
  if(isDefaultFree){
    ...
    // v1030: built-in Radio is sourced exclusively from Radio-Online.my.
...
function catalogMovies(){
  const current=state.channels.filter(c=>mediaType(c)==='movies');
  if(current.length){if(!state.movieCatalogSnapshot.length)state.movieCatalogSnapshot=current.slice();return current}
  return state.movieCatalogSnapshot.length?state.movieCatalogSnapshot:embeddedMovieCatalog();
}
...
function replaceRadioCatalog(radios,statusText=''){
  // v1085: when the live Radio-Online.my catalogue refresh arrives, keep any
  // bundled local artwork already loaded for the same slug. This prevents a
  // fast local icon from being replaced by a slower remote image request.
  const bundledBySlug=new Map(state.channels.filter(c=>mediaType(c)==='radio').map(c=>[String(c?.slug||'').toLowerCase(),String(c?.logo||'')]).filter(x=>x[0]&&/\/channel-icons\//i.test(x[1])));
  const rows=Array.isArray(radios)?radios.filter(Boolean).map(applyLocalLiveArtwork).map(c=>{
    const local=bundledBySlug.get(String(c?.slug||'').toLowerCase())||'';
    if(!local)return c;
    const upgrade=String(c.remoteLogo||c.logo||'').trim();
    return {...c,logo:local,remoteLogo:upgrade!==local?upgrade:'',fallbackLogo:''};
  }):[];
  if(!rows.length)return false;
  rows.filter(c=>!c.webOnly).forEach(c=>state.trustedDemoUrls.add(c.url));
...
function replaceMovieCatalog(movies,statusText=''){
  const rows=rememberMovieCatalog(Array.isArray(movies)?movies.filter(Boolean):[]);
  if(!rows.length)return false;
...
async function loadInstantLocalLive(){
  const raw=await localTextGet('./data/free.m3u?v=1085',5000);
  return parseM3U(raw).map(applyLocalLiveArtwork).map(c=>({...c,kind:'live'}));
}
...
async function boot(){
  ...
  // v1036: local Movies are first-paint data. Do not wait for a sleeping Render
  // instance before showing the real movie cards. Live 1Tube metadata refreshes later.
  const localMoviePromise=loadLocalMovieCatalog().catch(e=>{
    console.warn('AZOBSSTV instant local Movies load:',e?.message||e);
    return[];
  });
  const remoteMoviePromise=custom?Promise.resolve([]):loadRemoteMovieCatalog().catch(e=>{
    console.warn('AZOBSSTV background 1Tube Movies sync:',e?.message||e);
    return[];
  });

  // v1036: Movies are available synchronously from the embedded snapshot before
  // any Live TV/Radio fetch is awaited. This prevents an instant tab switch from
  // seeing "No content found" and makes Movies resilient to background races.
  let localLive=[];
  let localRadio=[];
  let localMovies=rememberMovieCatalog(embeddedMovieCatalog());
  if(localMovies.length){
    state.channels=[...localMovies];
    refreshCatalogView('Movies ready • loading TV + Radio…');
  }

  // Load tiny local Live TV next. Radio fallback is allowed to arrive later and
  // can no longer hold the first Movies render hostage.
  try{
    localLive=await loadInstantLocalLive();
  }catch(e){
    console.warn('AZOBSSTV instant local Live TV unavailable:',e?.message||e);
  }
  state.channels=[...localLive,...localMovies];
  ...
  // already visible, so even a slow/failed static fetch cannot blank the tab.
  localMoviePromise.then(movies=>{if(movies.length)replaceMovieCatalog(movies,$('#playlistStatus').textContent||'AZOBSSTV')});

  loadRadioOnlineFallback().then(rows=>{
    localRadio=rows;
    if(rows.length)replaceRadioCatalog(rows,$('#playlistStatus').textContent||'AZOBSSTV');
  }).catch(e=>console.warn('AZOBSSTV instant Radio-Online.my fallback unavailable:',e?.message||e));

  // Local Movies are already present in the first paint. Refresh them only when
  // the live 1Tube metadata request completes successfully in the background.
  remoteMoviePromise.then(movies=>{
  ...
  if(!state.channels.length){
    const [anime,remote,localMoviesFallback,remoteMoviesFallback]=await Promise.all([animePromise,remoteLivePromise,localMoviePromise,remoteMoviePromise]);
    const movies=(remoteMoviesFallback&&remoteMoviesFallback.length)?remoteMoviesFallback:(localMoviesFallback||[]);
