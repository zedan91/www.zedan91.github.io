...
async function showAnimeEpisodePlayer(series,ep){
  ...
  if(check.embeddable&&check.embed_url&&isSafeAnimeEmbedPageUrl(check.embed_url)){
    ...
    if(status)status.textContent=check?.reason==='no-valid-embeddable-html-player'?'No valid embeddable player found':'Embedding blocked by source';
  }
}
...
async function loadPlaylist(url,name='AZOBSSTV Free'){
  ...
  if(isDefaultFree&& !parsed.length){
    try{
      const raw=await textDirect('./data/free.m3u?v=1004',15000);
      parsed=parseM3U(raw);
      if(parsed.length){
...
window.addEventListener('DOMContentLoaded',()=>{bindEnglishAZOBSSTVNavigation();bind();startHeroSideSync();boot();if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=1004').catch(()=>{});if(document.visibilityState==='visible')startForegroundLoops()});
})();
