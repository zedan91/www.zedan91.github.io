...
async function loadAnimeCatalog(){try{const data=await jget('./data/anime-catalog.json?v=1003',18000);const rows=Array.isArray(data?.items)?data.items:[];return rows.map((x,index)=>{const url=String(x?.sourcePage||x?.url||'').trim();const name=String(x?.name||`Anime ${index+1}`).trim();if(!url||!name)return null;const categories=Array.isArray(x?.categories)?x.categories.map(v=>String(v||'').trim()).filter(Boolean):[];return{name,logo:String(x?.logo||'').trim(),id:String(x?.id||x?.slug||`anime-${index+1}`).trim(),group:'Anime',kind:'series',categories,year:x?.year??null,rating:'',episodeCount:Number(x?.episodeCount||0)||0,episodeBase:String(x?.episodeBase||'').trim(),episodes:[],sourceProvider:String(x?.sourceProvider||'123animehub'),sourcePage:url,webOnly:true,mode:'web',officialUrl:url,altUrl:'',url,headers:{userAgent:'',referer:'',origin:'',authorization:''},drm:{licenseType:'',licenseKey:''},slug:String(x?.slug||'').trim(),status:String(x?.status||''),animeType:String(x?.type||''),tags:Array.isArray(x?.tags)?x.tags:[]}}).filter(Boolean)}catch(e){console.warn('AZOBSSTV anime catalogue fallback:',e?.message||e);return[]}}
...
function renderAnimeSideEpisodes(series,currentUrl=''){
  ...
  box.innerHTML=eps.map((ep,index)=>{
...
async function checkAnimeEmbed(url){
  try{
    ...
    if(data&&data.ok&&data.embeddable&&data.embed_url)return data;
...
async function showAnimeEpisodePlayer(series,ep){
  ...
  if(check.embeddable&&check.embed_url){
    setAnimeEmbedCompactMode(false);
    if(status)status.textContent='Displayed in AZOBSSTV • public external player';
    frame.hidden=false;
...
async function loadPlaylist(url,name='AZOBSSTV Free'){
  ...
  if(isDefaultFree&& !parsed.length){
    try{
      const raw=await textDirect('./data/free.m3u?v=1003',15000);
      parsed=parseM3U(raw);
      if(parsed.length){
...
window.addEventListener('DOMContentLoaded',()=>{bindEnglishAZOBSSTVNavigation();bind();startHeroSideSync();boot();if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=1003').catch(()=>{});if(document.visibilityState==='visible')startForegroundLoops()});
})();
