...
function requireSignIn(feature='Favorites'){if(isSignedIn())return true;alert(`Sila sign in ke akaun AZOBSS untuk menyimpan ${feature}.`);return false}
...
function updateFavoriteAvailability(){const signed=isSignedIn();const current=$('#favCurrentBtn');if(current){current.classList.toggle('auth-locked',!signed);current.setAttribute('aria-disabled',signed?'false':'true');current.title=signed?'Favorite':'Sign in untuk simpan Favorite'}$$('.fav-mini,.channel-rail-fav').forEach(btn=>{btn.classList.toggle('auth-locked',!signed);btn.setAttribute('aria-disabled',signed?'false':'true');btn.title=signed?'Favorite':'Sign in untuk simpan Favorite'})}
...
function syncHeroSideHeight(){
  const player=document.querySelector('.player-card'),side=$('#heroSide');if(!player||!side||matchMedia('(max-width:800px)').matches){if(side)side.style.height='';return}
  side.style.height=Math.round(player.getBoundingClientRect().height)+'px';
}
...
function setOfficialWide(enabled){
  ...
  if(hotspot){const label=state.officialWide?'Kembali ke saiz biasa':'Lebarkan player';hotspot.setAttribute('aria-pressed',state.officialWide?'true':'false');hotspot.setAttribute('aria-label',label);hotspot.title=label}
