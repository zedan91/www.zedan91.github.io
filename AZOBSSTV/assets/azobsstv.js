...
function syncHeroSideHeight(){
  ...
  if(!player||!side||matchMedia('(max-width:800px)').matches||hero?.classList.contains('anime-compact-hero')){
    if(side)side.style.height='';
    return;
  }
...
window.addEventListener('DOMContentLoaded',()=>{bindEnglishAZOBSSTVNavigation();bind();startHeroSideSync();boot();if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=998').catch(()=>{});if(document.visibilityState==='visible')startForegroundLoops()});
})();
