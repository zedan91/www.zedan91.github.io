  ...
  return async function handleAZOBSSTV(req, res, parsed) {
    ...
    try {
      if (pathname === '/api/azobsstv/health' && (req.method === 'GET' || req.method === 'HEAD')) {
        sendJson(res, 200, { ok: true, service: 'AZOBSSTV', version: '1.0.974', firestore: !!getDb(), stream_query_parser: 'node-url-parse-compatible', rtm_referer: true, rtm_origin: true, playback_strategy: 'official-player-for-protected-rtm', time: Date.now() });
        return true;
      }
