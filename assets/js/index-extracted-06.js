...
(function(){
  ...
  function getCurrentUserObject(){
    try{
      return JSON.parse(sessionStorage.getItem('azobssCurrentUser') || 'null');
    }catch(e){
      return null;
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', restorePurchaseRecordsPanel);
  window.addEventListener('load', restorePurchaseRecordsPanel);
  window.addEventListener('storage', restorePurchaseRecordsPanel);
  setTimeout(restorePurchaseRecordsPanel, 300);
  setTimeout(restorePurchaseRecordsPanel, 1200);
  setTimeout(restorePurchaseRecordsPanel, 2500);
})();
