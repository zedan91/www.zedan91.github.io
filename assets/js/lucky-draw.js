// Lucky Draw extracted JS only
// Auth system intentionally left inline/original.


/* ===== Lucky Draw JS Block 1 ===== */
import { initializeApp } from 'https://www.gstatic.com/firebasejs/12.7.0/firebase-app.js';
    ...
    function getFilteredLuckyDrawParticipants() {
      ...
      const rows = allLuckyDrawParticipants.filter(function (entry) {
          ...
          ? rows.map(function (entry, index) {
              return `
                <div class="lucky-draw-participant-card">
                  <strong>${index + 1}. ${escapeLuckyDrawText(entry.name || entry.usernameKey || 'User')}</strong>
                  <span>${escapeLuckyDrawText(entry.monthKey || 'Lucky Draw')} • ${escapeLuckyDrawText(entry.usernameKey || '-')}</span>
                </div>
              `;
            }).join('')
    ...
    async function loadLuckyDrawParticipants() {
      ...
      try {
        ...
        rows.sort(function (a, b) {
          return (b.joinedAtMs || b.createdAtMs || 0) - (a.joinedAtMs || a.createdAtMs || 0);
        });
    ...
    function spinLuckyDrawNames(participants, finalWinner, monthKey) {
      ...
      function nextTick() {
        ...
        if (tick >= maxTicks) {
          ...
          luckyDrawSpinnerName.textContent = `🎉 ${getWinnerDisplayName(finalWinner)} 🎉`;
    ...
    function updateLuckyDrawCountdown() {
      ...
      if (luckyDrawTargetText) {
        luckyDrawTargetText.innerHTML = `Rolling Lucky Draw akan bermula pada<br>${target.toLocaleDateString('ms-MY')} • 10:00 PM (UTC+08:00)`;
      }
    ...
    async function testLuckyDrawSpinNow() {
      ...
      function testTick() {
        ...
        if (tick >= maxTicks) {
          ...
          luckyDrawSpinStage.classList.remove('is-spinning');
          luckyDrawSpinStage.classList.add('has-winner');
          renderLuckyDrawRollingItems([`TEST 🎉 ${getWinnerDisplayName(randomWinner)} 🎉`], 0);
          luckyDrawSpinnerName.textContent = `TEST 🎉 ${getWinnerDisplayName(randomWinner)} 🎉`;
          playLuckyDrawWinnerSound();
          createLuckyDrawConfetti();
    ...
    function getLuckyDrawPrizePlaceholder() {
      return 'data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 900 520'%3E%3Cdefs%3E%3CradialGradient id='g' cx='50%25' cy='42%25' r='65%25'%3E%3Cstop offset='0%25' stop-color='%2316a34a' stop-opacity='.34'/%3E%3Cstop offset='58%25' stop-color='%230f172a'/%3E%3Cstop offset='100%25' stop-color='%23020617'/%3E%3C/radialGradient%3E%3Cfilter id='s' x='-30%25' y='-30%25' width='160%25' height='160%25'%3E%3CfeDropShadow dx='0' dy='18' stdDeviation='18' flood-color='%23000000' flood-opacity='.45'/%3E%3C/filter%3E%3C/defs%3E%3Crect width='900' height='520' fill='url(%23g)'/%3E%3Cellipse cx='450' cy='425' rx='190' ry='34' fill='%23000000' opacity='.28'/%3E%3Cg filter='url(%23s)'%3E%3Crect x='305' y='235' width='290' height='190' rx='28' fill='%237c3aed'/%3E%3Crect x='285' y='200' width='330' height='70' rx='24' fill='%238b5cf6'/%3E%3Crect x='430' y='200' width='42' height='225' fill='%23facc15'/%3E%3Crect x='285' y='225' width='330' height='32' fill='%23fde047' opacity='.92'/%3E%3Cpath d='M448 198 C390 140 330 145 328 188 C326 226 392 228 448 198Z' fill='%23fbbf24'/%3E%3Cpath d='M454 198 C512 140 572 145 574 188 C576 226 510 228 454 198Z' fill='%23facc15'/%3E%3Ccircle cx='545' cy='320' r='52' fill='%230f172a' opacity='.82'/%3E%3Ctext x='545' y='342' text-anchor='middle' font-size='72' font-family='Arial' font-weight='900' fill='%23fde047'%3E?%3C/text%3E%3C/g%3E%3Ctext x='450' y='92' text-anchor='middle' font-size='58' font-family='Arial' font-weight='900' fill='%2322c55e'%3ELUCKY DRAW%3C/text%3E%3Ctext x='450' y='145' text-anchor='middle' font-size='25' font-family='Arial' font-weight='700' fill='%23cbd5e1'%3EHadiah masih misteri%3C/text%3E%3C/svg%3E';
    }
  ...
  window.azobssTestBackendNow = async function(){
    ...
    try{
      ...
      if(res.ok && data && data.ok){
        if(status) status.textContent = 'Backend OK ✅ ' + url;
        alert('Backend OK ✅\\n' + url);
        return;
      }
      ...
      if(status) status.textContent = 'Backend test gagal ❌ Buka /api/health manual untuk check.';
      alert(msg);
