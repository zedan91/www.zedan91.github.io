// AZOBSS Cloudflare R2 Download + Tech Vault Upload Gateway - Patch 717
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname === "/health") {
        return json({
          ok: true,
          patch: "AZOBSS_R2_WORKER_TECH_VAULT_UPLOAD_718_20260803",
      ...
      const uploadRequest = (() => { try { return new URL(request.url).pathname.startsWith("/vault-upload/"); } catch (_) { return false; } })();
      const extra = uploadRequest ? Object.fromEntries(corsHeaders()) : {};
...
async function handleVaultUpload(request, env, url) {
  ...
  return json({ ok:true, key:payload.key, name, size:storedSize || expectedSize, etag:stored?.httpEtag || "", patch:"717" }, 200, corsHeaders());
}
