...
(function(){
  ...
  function iconFor(title,cat){
    var t=String(title||'').toLowerCase();
    if(/frypan|wok|cookware|kitchen|tefal|spatula|blender|grinder|chopper|mixer|mincer|pan|pot|air fryer/.test(t)) return 'ðŸ³';
    if(/vacuum|cleaner|mop|dust/.test(t)) return 'ðŸ§¹';
    if(cat==='computer') return 'ðŸ’»';
    if(cat==='mobile') return 'ðŸ“±';
    if(cat==='automotive') return 'ðŸš—';
    if(cat==='baby') return 'ðŸ§¸';
    if(cat==='sports') return 'ðŸ‹ï¸';
    if(cat==='fashion') return 'ðŸ‘œ';
    return 'ðŸ›’';
  }
  ...
  function copyScript(){
    ...
    if(navigator.clipboard && window.isSecureContext){
      navigator.clipboard.writeText(s).then(function(){status('âœ… Script copied. Buka Shopee â†’ F12 Console â†’ paste â†’ Enter. JSON akan auto download.',false);}).catch(function(){fallbackCopy(s);});
    }else fallbackCopy(s);
  }
  function fallbackCopy(s){
    ...
    try{document.execCommand('copy');status('âœ… Script copied. Buka Shopee â†’ F12 Console â†’ paste â†’ Enter.',false);}catch(e){status('Copy gagal. Sila cuba browser lain atau copy manual.',true);}finally{ta.remove();}
  }
  ...
  function apply(raw){
    ...
    status('âœ… JSON imported. Form auto filled daripada data Shopee. Sila semak sebelum Save.',false);
  }
