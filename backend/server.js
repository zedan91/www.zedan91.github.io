...
function azPremiumClientKey(req){ return azHashDownloadValue(azPremiumClientIp(req)); }
...
async function azHandlePremiumDownloadSession(req,res){ const sessionId=String(req.params.sessionId||"").replace(/[^a-zA-Z0-9_-]/g,""); const now=Date.now(); const session=findPremiumDownloadSession(sessionId); if(!session) return res.status(404).send("Download session not found."); if(session.status!=="active"||Number(session.expiresAt||0)<=now){ updatePremiumDownloadSession(sessionId,{status:"expired",expiredAt:new Date().toISOString(),expiredAtMs:now}); return res.status(410).send("Download session expired."); } if(session.clientKey&&session.clientKey!==azPremiumClientKey(req)) return res.status(403).send("This download session belongs to another network/device."); updatePremiumDownloadSession(sessionId,{lastSeenAt:new Date().toISOString(),lastSeenAtMs:now,requestCount:Number(session.requestCount||0)+1,rangeRequestCount:Number(session.rangeRequestCount||0)+(req.headers.range?1:0),lastRange:req.headers.range?String(req.headers.range).slice(0,120):""}); try{ if(session.sourceType==="local"||String(session.sourceTarget||"").startsWith("/")) await azStreamLocalPremiumSession(req,res,session); else await azStreamRemotePremiumSession(req,res,session); if(!req.headers.range) updatePremiumDownloadSession(sessionId,{status:"completed",completedAt:new Date().toISOString(),completedAtMs:Date.now()}); }catch(err){ console.error("AZOBSS legacy secure download stream failed:",err&&err.stack||err); updatePremiumDownloadSession(sessionId,{lastError:err&&err.message?err.message:String(err),lastErrorAt:new Date().toISOString()}); if(!res.headersSent) res.status(500).send("Download failed. Please contact admin."); else { try{res.destroy(err);}catch{} } } }
